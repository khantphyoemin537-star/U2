...
SPAM_CATCH_MUTE_SECONDS = 480  # 8 minutes — how long /fuck stays blocked, GLOBALLY across
# every group the user is in (not just the group where they spammed). 🩹 CHANGED (per owner
# request): /who, /w, /waifu are now EXEMPT from this mute — a spam-muted user can still
# identify a spawn, they just can't claim it.
...
async def spam_detection_and_mute(event):
    ...
    if user_id in user_mute_until and now < user_mute_until[user_id]:
        if _extract_command_word(event.text) in ['/fuck', '/obtain']:
            try:
                await event.delete()
            except:
                pass
        ...
        try:
            ...
            await event.respond(
                bq(f"<b>Notice:</b> {mention}, that's enough. "
                   f"You've been blocked from /fuck <b>in every group</b> for {mute_minutes} minutes. "
...
async def gift_asset_handler(event):
    if not event.is_reply:
        return await event.reply("❌ <b>ဂီဖ်ပေးချင်တဲ့ လူရဲ့ message ကို Reply ပြန်ပြီး ခိုင်းပါ။</b>", parse_mode='html')
    ...
    if sender_id == receiver_id:
        return await event.reply("❌ <b>ကိုယ့်ကိုယ်ကို ဂီဖ်ပေးလို့ မရဘူးနော်။</b>", parse_mode='html')

    char_data = await characters_base_col.find_one({"char_id": char_id})
    if not char_data:
        return await event.reply("❌ <b>ဒီ ID နဲ့ character ကို ရှာမတွေ့ဘူး။</b>", parse_mode='html')
    ...
    if sender_id == OWNER_ID:
        ...
        if gift_count_used >= ADDED_OWNER_GIFT_LIMIT_PER_CARD:
            return await event.reply(
                f"❌ <b>ဒီ card ကို အများဆုံး {ADDED_OWNER_GIFT_LIMIT_PER_CARD} ကြိမ် ပေးပြီးသားပါ။</b>",
                parse_mode='html'
            )
    else:
        sender_doc = await users_catcher_col.find_one({"user_id": sender_id})
        ...
        if not char_item:
            return await event.reply(f"❌ <b>ဒီ card က မင်းပိုင်တာ မဟုတ်လို့ ပေးလို့မရဘူး။</b>", parse_mode='html')
    ...
    buttons = [
        [
            Button.inline("💎 အတည်ပြုမယ်", data=f"gift_confirm_{sender_id}_{receiver_id}_{char_id}"),
...
async def gift_callback_handler(event):
    ...
    if event.sender_id != sender_id:
        return await event.answer("❌ ဒါက မင်းအတွက် မဟုတ်ဘူး!", alert=True)
    if not claim_single_tap(event):
        return await event.answer("⏳ လုပ်ဆောင်နေပြီ...", alert=False)
    ...
    if action == "cancel":
        ...
        try:
            await event.edit(
                f"…<b>စိတ်ပြောင်းသွားပြီနော်။</b>\n\n"
                f"Card <code>{display_char_id(char_id)}</code> က မင်းဆီမှာပဲ ဆက်ရှိနေမယ်။\n"
                f"🎁 နောက်တစ်ခါမှ ပြန်ပေးလိုက်ပေါ့",
                parse_mode='html',
                buttons=None
            )
            await event.answer("ဂီဖ် ပယ်ဖျက်လိုက်ပါပြီ။", alert=True)
        except Exception as e:
            await report_system_error("gift_callback_handler (cancel)", f"{e}\nData: {event.data}")
            try:
                await event.answer("❌ တစ်ခုခု မှားသွားပါပြီ!", alert=True)
            except Exception:
                pass
        return
    ...
    try:
        if sender_id == OWNER_ID:
            ...
            if not char_data:
                await event.edit(
                    f"❌ <b>…သူ ပျောက်သွားပြီ။</b>\n\n"
                    f"Character <code>{display_char_id(char_id)}</code> ရှိမနေတော့ဘူး။",
                    parse_mode='html',
                    buttons=None
                )
                await event.answer("Card မတွေ့ဘူး။", alert=True)
                return
            ...
            if gift_count_used >= ADDED_OWNER_GIFT_LIMIT_PER_CARD:
                ...
                await event.answer("ပေးလို့ရတဲ့ အကြိမ်ရေ ကုန်သွားပြီ။", alert=True)
                return
            char_rarity = char_data.get("rarity", "Unknown")
            gift_tier = classify_rarity(char_rarity)
            await users_catcher_col.update_one(
            ...
            if not char_item:
                await event.edit(
                    f"❌ <b>…card က မရှိတော့ဘူး။</b>\n\n"
                    f"Card <code>{display_char_id(char_id)}</code> က မင်းဆီမှာ မရှိတော့ဘူး။\n"
                    f"🎲 ကံနည်းနည်း ဆိုးသွားပုံရတယ်",
                    parse_mode='html',
                    buttons=None
                )
                await event.answer("Card မတွေ့ဘူး။", alert=True)
        ...
        # 🩹 FIX: this was `f""` — an empty string contributing nothing, not even a newline —
        # so the header ran straight into "From:" with no blank line, while every other gap in
        # this same message (and every other gift message in this file) uses a blank line.
        # That's the "torn/uneven" look being reported: fixed to `f"\n"` so the spacing matches.
        success_text = (
            f"◈ <b>ဂီဖ် အောင်မြင်ပါပြီ 🎉</b>\n\n"
            f"📤 <b>ပေးသူ:</b> {s_mention}\n"
            f"📥 <b>ရရှိသူ:</b> {r_mention}\n"
        ...
        # 🩹 FIX ("caption cut off" bug — same class as /harem's "Message was too long" crash and
        # /profile's caption fix): this message has the character's photo attached, so editing
        # it edits a CAPTION (1024-UTF-16-unit cap), not a full message (4096). Two long display
        # names (s_mention/r_mention) plus a long character name can together blow past that and
        # get cut off by Telegram. There's no way to turn a media message back into a text-only
        # one via .edit(), so when the full text won't safely fit as a caption, shrink the
        # caption to a short placeholder and post the full details as a normal follow-up message
        # instead — a plain message has the much higher 4096-char budget.
        CAPTION_SAFE_LIMIT = 1024
        if utf16_len(success_text) <= CAPTION_SAFE_LIMIT:
            await event.edit(success_text, parse_mode='html', buttons=None)
        else:
            try:
                await event.edit("◈ <b>ဂီဖ် အောင်မြင်ပါပြီ 🎉</b> (အသေးစိတ်ကို အောက်မှာကြည့်ပါ ⬇️)", parse_mode='html', buttons=None)
            except Exception:
                pass
            await event.respond(success_text, parse_mode='html')
        await event.answer("ဂီဖ် ပေးလိုက်ပါပြီ!", alert=True)
        ...
        try:
            await event.edit(
                "❌ <b>ဂီဖ် မအောင်မြင်ပါ — Internal error ဖြစ်နေလို့ နောက်မှ ထပ်ကြိုးစားပါ။</b>",
                parse_mode='html',
                buttons=None
            )
            await event.answer("❌ တစ်ခုခု မှားသွားပါပြီ!", alert=True)
