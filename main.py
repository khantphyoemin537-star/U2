...
async def find_character_by_media(media_msg):
    ...
    live spawn): checks a Redis cache keyed by the raw Telegram file id FIRST, before touching
    the download/hash/scan pipeline at all. The very first /who on a brand-new file still pays
    the full cost (that part is unavoidable — nothing can know what an image contains without
    looking at it), but every subsequent /who on that SAME file — the common case for a
    reposted or forwarded spawn image that several different people ask about — now resolves
    in a single Redis round-trip instead of re-downloading and re-hashing from scratch."""
    media_key = get_media_identity_key(media_msg)
    if media_key:
        try:
            cached_char_id = await redis_client.get(f"cache:mediaid:{media_key}")
        except Exception as e:
            print(f"find_character_by_media cache read error: {e}")
            cached_char_id = None
        if cached_char_id is not None:
            if cached_char_id == "__NONE__":
                return None
    ...
    if media_key:
        try:
            await redis_client.setex(f"cache:mediaid:{media_key}", MEDIA_IDENTIFY_CACHE_TTL,
                                      best_match["char_id"] if best_match else "__NONE__")
        except Exception as e:
            print(f"find_character_by_media cache write error: {e}")
    return best_match
...
async def _xbot_identify_fallback_and_reply(event, media_msg, known_source_bot=None):
    ...
    media_key = get_media_identity_key(media_msg)
    cache_field = f"cache:xbotmediaid:{media_key}" if media_key else None
    name = source_bot = None
    if cache_field:
        try:
            cached_raw = await redis_client.get(cache_field)
        except Exception as e:
            print(f"_xbot_identify_fallback_and_reply cache read error: {e}")
            cached_raw = None
        if cached_raw is not None:
            if cached_raw == "__NONE__":
                return await event.reply("❓ <b>This one isn't recognized.</b>", parse_mode='html')
            try:
                cached_doc = json.loads(cached_raw)
                name, source_bot = cached_doc.get("name"), cached_doc.get("source_bot")
            except Exception:
                name = None  # malformed/legacy cache entry — fall through and recompute fresh
        ...
        if cache_field:
            try:
                await redis_client.setex(
                    cache_field, XBOT_MEDIA_IDENTIFY_CACHE_TTL,
                    json.dumps({"name": doc["name"], "source_bot": doc.get("source_bot", "unknown")}) if doc else "__NONE__"
                )
            except Exception as e:
                print(f"_xbot_identify_fallback_and_reply cache write error: {e}")
        if not doc:
            return await event.reply("❓ <b>This one isn't recognized.</b>", parse_mode='html')
