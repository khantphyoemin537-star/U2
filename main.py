...
# being a member of MARKET_FORCE_SUB_INVITE_LINK's group — checked independently of, and in
# addition to, the site-wide FORCE_SUB_CHAT_ID gate above (a different group). Since that gate
# only covers slash-commands matched by raw text, and half of the market's surface is inline
# buttons, _market_require_group_member() is called explicitly at the top of every command AND
# every callback handler below, rather than being added to FORCE_SUB_GATED_COMMANDS.
...
MARKET_FORCE_SUB_INVITE_LINK = "https://t.me/+QdhGvV5-rOs4NGY9"  # 🚦 required-join group for the market — see note above
...
_market_force_sub_chat_id = None  # resolved once from MARKET_FORCE_SUB_INVITE_LINK, cached forever (see get_force_sub_invite_link above for the same pattern)

async def _get_market_force_sub_chat_id():
    global _market_force_sub_chat_id
    if _market_force_sub_chat_id is not None:
        return _market_force_sub_chat_id
    try:
        entity = await bot1.get_entity(MARKET_FORCE_SUB_INVITE_LINK)
        _market_force_sub_chat_id = entity.id
    except Exception as e:
        print(f"⚠️ Market force-sub group resolve failed (is bot1 a member of that group yet?): {e}")
    return _market_force_sub_chat_id
...
async def is_market_force_sub_member(user_id):
    ...
    if cached and now < cached[1]:
        return cached[0]
    chat_id = await _get_market_force_sub_chat_id()
    if chat_id is None:
        return False  # can't verify yet — see the ⚠️ SETUP REQUIRED note above; fails closed, not open
    try:
        perms = await bot1.get_permissions(chat_id, user_id)
        is_member = perms is not None
    except Exception:
        is_member = False
...
async def _market_place_bid(conv, listing_id, bidder_id, amount):
    ...
    if prev_bidder_id and prev_amount > 0:
        ...
        try:
            await bot1.send_message(
                prev_bidder_id,
                f"📢 <b>You've been outbid!</b> Someone bid <code>{amount:,}</code> ccm on "
...
async def market_sell_command(event):
    ...
    if price < MARKET_MIN_STARTING_PRICE:
        return await event.reply(f"❌ <b>Minimum starting price is {MARKET_MIN_STARTING_PRICE:,} ccm.</b>", parse_mode='html')
    ...
    if not char_item:
        return await event.reply("❌ <b>…she's not yours to sell</b> (or she's already listed/otherwise unavailable).", parse_mode='html')

    rarity = char_item.get("rarity", char_data.get("rarity", "Unknown"))
    confirm_text = (
...
async def _settle_market_listing(listing):
    ...
    if winner_id and final_bid > 0:
        ...
        proceeds = final_bid - fee  # the fee is simply never credited anywhere — see the 💰 ccm sink note up top
