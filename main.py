...
async def handle_name_search_inline_query(event, query_text):
    """search.<term> — free-text character NAME search across the whole roster (unlike dex.,
    this needs no exact category match, and unlike harem./collected. it isn't limited to what
    the querying user owns). Substring + case-insensitive, so typing "mikasa" matches a
    character actually named "Mikasa Ackerman" — no need to type the full name. Results sorted
    rarity high→low (ties broken alphabetically), same media-attached inline card style as the
    dex gallery (handle_dex_inline_query above), which this is closely modeled on."""
    builder = event.builder
    term = query_text.split(".", 1)[1].strip() if "." in query_text else ""
    if not term:
        return await event.answer([], cache_time=0)
    user_id = getattr(event, 'sender_id', None)
    if not user_id:
    ...
    matches = [c for c in all_chars if term_lower in (c.get("name") or "").lower()]
    matches.sort(key=lambda x: (-_rarity_weight_for_sort(x.get("rarity", "")), x.get("name", "").lower()))
    results = []
    page_chars = matches[:50]
    # Batch-fetch (see handle_harem_inline_query for why this matters — one call instead of
    # up to 50 sequential ones); further reduced to only-the-misses by the cache inside
    # get_char_display_media_batch.
    try:
    ...
    for card in page_chars:
        ...
        try:
            caption = (
                f"Character Name🐇<b>{escape_html(card['name'])}</b>\n"
                f"ID <code>{display_char_id(cid)}</code>\n"
                f"🏞️<b>Category:</b> {escape_html(card.get('category') or 'Unknown Series')}\n"
                f" <b>Rarity:</b> {card.get('rarity', '')}\n"
                f"{artist_line(card)}"
                f"{'✅ You caught this!' if owned else '❓ Not caught yet'}"
            )
...
async def dex_search_handler(event):
    ...
    if not matched_cat:
        return await event.reply(
            ...
            parse_mode='html',
            buttons=[[Button.switch_inline(f"🔍 Search \"{category_query}\"", query=f"search.{category_query}", same_peer=True)]]
        )
