...
async def harem_handler(event):
    ...
    if event.is_reply:
        ...
        if args:
            ...
            if args.startswith('@'):
                try:
                    entity = await bot1.get_entity(args)
    ...
    if target_user_id is None:
        ...
        try:
            target_mention = await get_mention(bot1, target_user_id)
        except Exception:
...
async def send_harem_overview(event, target_user_id, viewer_id=None, page=1, edit_msg_id=None):
    ...
    try:
        target_entity = await bot1.get_entity(target_user_id)
    ...
    buttons.append([Button.switch_inline("📸 Gases", query=f"harem.{target_user_id}", same_peer=False)])
...
async def send_leaderboard(event, scope):
    ...
    for i, doc in enumerate(top):
        ...
        mention = await get_mention(bot1, uid)
...
async def help_handler(event):
    help_text = (
        ...
        "/w, /who, /waifu — reply to the spawn message to reveal it\n"
