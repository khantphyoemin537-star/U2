...
# catch_bot's @username — used ONLY for the /start warm-up (a fresh userbot session can't
# resolve a bare numeric id until it has "met" the bot). Override with the env var if this
# guess is wrong; the numeric CATCH_BOT_ID is still tried as a fallback.
SYNC_TARGET_BOT_USERNAME = os.environ.get("SYNC_TARGET_BOT_USERNAME", "Character_Catcher_Bot")
...
async def _nsync_warmup_one(client, label: str) -> bool:
    ...
    username = SYNC_TARGET_BOT_USERNAME
    if not username.startswith("@"):
        username = "@" + username
