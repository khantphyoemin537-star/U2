...
async def auto_import_character_from_catchbot(msg, chat_id, source_bot, silent=False):
    ...
    # Avoid importing the same character twice (case-insensitive exact name match) — e.g. if
    # the bot restarts and /xbotscan or the bulk-import job re-walks history it's already
    # imported from.
    existing = await characters_base_col.find_one({"name": {"$regex": f"^{re.escape(info['name'])}$", "$options": "i"}})
    if existing:
...
async def _find_imported_character(name: str):
    """Case-insensitive exact-name lookup, restricted to characters we auto-imported from a
    cross-bot source (auto_imported_from set) — never touches a manually-/addchar'd character
    that happens to share a name. Used by every sync function below."""
...
async def _maybe_handle_xbot_rename(msg, caption: str) -> bool:
    ...
    if old_name and new_name and (msg.photo or msg.video or msg.document):
        ...
        imported = await _find_imported_character(old_name)
        if imported:
            await characters_base_col.update_one({"_id": imported["_id"]}, {"$set": {"name": new_name}})
            await invalidate_character_caches()
            await bot1.send_message(
                OWNER_ID,
...
async def _maybe_sync_event_change(caption: str) -> bool:
    """Checked alongside the rename check above, same placement/timing. Doesn't need media —
    an event tag change doesn't touch the character's image."""
    ...
    if new_event.strip() in ("None", "-", ""):
        new_event = "General"
    imported = await _find_imported_character(name)
    if imported:
...
async def _maybe_sync_catchbot_character_change(msg, chat_id, caption: str) -> bool:
    ...
    catch_bot' checks — rename, event change, rarity change, image update — called from BOTH
    listeners before their generic/fallback handling. Short-circuits on the first match, since
    a post is only ever one of these kinds. Returns True if any of them handled this message."""
    if await _maybe_handle_xbot_rename(msg, caption):
        return True
    if await _maybe_sync_event_change(caption):
        return True
...
async def _run_bulk_import(chat_ids, status_chat_id=None, status_msg_id=None):
    ...
    counts = {
        ...
        "skipped_duplicate": state.get("skipped_duplicate", 0), "errors": state.get("errors", 0)
    }
    await bot_settings_col.update_one(
                ...
                if status_chat_id and status_msg_id and counts["checked"] % 25 == 0:
                    try:
                        await bot1.edit_message(
                            ...
                            f"⏭️ Already had: {counts['skipped_duplicate']}"
                            + (f"\n⚠️ Errors: {counts['errors']}" if counts['errors'] else ""),
                            parse_mode='html'
                        )
                    except Exception:
                        pass
...
async def xbot_import_status_command(event):
    ...
    await event.reply(
        ...
        f"Status: {running}\n"
        f"📨 Checked: {state.get('checked', 0)}\n"
        f"🆕 Imported: {state.get('imported', 0)}\n"
        f"⏭️ Already had: {state.get('skipped_duplicate', 0)}\n"
        f"⚠️ Errors: {state.get('errors', 0)}\n"
...
async def send_paginated_harem(client, chat_id, user_id, page=1, edit_msg_id=None, viewer_id=None):
    ...
    # 🩹 FIX (per owner report): cards were sorted flat by rarity then name only, completely
    # ignoring category — so two cards from the same anime with different rarities ended up
    # scattered far apart instead of showing up together. Category is now the primary sort
    # key (alphabetical), with rarity then name breaking ties within each anime — no separate
    # header needed since every card block already carries its own "⚜️ Anime: X" line.
    sorted_cards = sorted(
        db_chars,
        key=lambda x: ((x.get("category") or "Unknown Series").lower(), -get_rarity_weight(x.get("rarity", "")), x.get("name", "").lower())
    )
    ...
    output_text = f"{mention}'s <b>Recent Waifus</b> - Page: <code>{page}/{total_pages}</code>\n"
    if rarity_filter:
...
async def render_profile_main_page(event, user_id, mention):
    ...
    header_line = "<b>This is your account</b>"
    text = (
        f"{header_line}\n"
        f""
...
# 🎁 GIFT (fixed)
# ==========================================
ADDED_OWNER_GIFT_LIMIT_PER_CARD = 3  # /addowner grantees: max times the SAME real-caught
...
async def gift_asset_handler(event):
    ...
    if sender_id == OWNER_ID:
        ...
        # 👑 Added owner (see /addowner): still needs a REAL catch to gift at all — but once
        # they have one, that same real catch can be gifted up to ADDED_OWNER_GIFT_LIMIT_PER_
        # CARD times total, not just once like an ordinary player's single physical copy. See
        # gift_callback_handler for where the quota is actually tracked/enforced.
...
async def gift_callback_handler(event):
    ...
    if sender_id == OWNER_ID:
        ...
        if gift_count_used >= ADDED_OWNER_GIFT_LIMIT_PER_CARD:
            ...
            return
        # 👑 Added-owner special case (see /addowner): does NOT remove the real harem copy —
        # the same real catch can be gifted up to ADDED_OWNER_GIFT_LIMIT_PER_CARD times total,
        # tracked via added_owner_gift_count, instead of being consumed after just one gift
        # like an ordinary player's single physical copy.
...
# and for /fav (no real /collect catch needed for either). Deliberately does NOT extend to
# /gift though — gift_asset_handler's and gift_callback_handler's ownership checks are keyed
# specifically to `== OWNER_ID`, not to added_owner_ids, so an added owner still has to
# actually own a REAL harem copy to gift it, exactly like an ordinary player. (Card trading —
# /trade, /buy, /sell, /market — no longer exists in this bot at all, so there's nothing left
# to restrict there; /gift is the only transfer command remaining.)
# ==========================================
...
async def add_owner_command(event):
    ...
    await event.reply(
        ...
        f"⚠️ <b>Gift ကတော့</b> <code>/collect</code> နဲ့ တကယ်ဖမ်းရထားတဲ့ကဒ်များသာ ပေးလို့ရပါမယ်။",
        parse_mode='html'
    )
