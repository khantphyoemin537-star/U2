...
# At level 1, /topspawn off (default, total weight 66): COMMON ≈48.5%, UNCOMMON ≈24.2%,
# RARE ≈12.1%, LEGENDARY ≈6.1%, MYSTICAL ≈3.0%, DIVINE ≈1.5%, SUPREME/CATAPHRACT/CROSSVERSE
# ≈1.5% each (a tiny sliver, on par with DIVINE rather than below it — TOP_TIER_WEIGHT_OFF is a
# flat residual chance, not itself part of the DIVINE-and-below curve above it). With /topspawn
# on (total weight 165), SUPREME/CATAPHRACT/CROSSVERSE jump to ≈60.6% each — exactly tied with
# DIVINE's own weight after the level multiplier. Tune the numbers above directly for a
# different curve; nothing else needs to change (get_effective_rarity_weights below reads these
        ...
        try:
            ...
            for char in characters_list:
                if not char.get("spawnable", True):
                    continue  # /addspecial CNFT cards — manual distribution only, never organic spawns
...
# activity instead of a Rarity 1-4 pick). Deliberately NO difficulty tiers, points, ranks, or
# leaderboard — every question pays the same flat 💰 ccm range regardless of how hard it is;
# see TRIVIA_CCM_MIN/MAX below. A correct tap WINS that amount; a wrong tap LOSES it (never
# below 0) — everyone gets exactly one attempt, right or wrong.
#
# Question bank is entirely owner-authored via /addtrivia (same private, step-by-step wizard
# as /addquiz above) — there's no built-in static question set, so trivia simply stays quiet
# (trigger_trivia_spawn no-ops) until at least one question has been added.
# ==========================================
TRIVIA_SPAWN_MIN_MSGS = 40   # min group messages between trivia rounds (per chat)
TRIVIA_SPAWN_MAX_MSGS = 60   # max group messages between trivia rounds (per chat) — averages ~50
...
TRIVIA_TIMEOUT_SECONDS = 60   # answering window, starting from the options reveal (not the post)
TRIVIA_CCM_MIN = 10
TRIVIA_CCM_MAX = 2000

async def _get_trivia_pool():
    return await trivia_bank_col.find({}, {"_id": 0}).to_list(length=None)

async def trigger_trivia_spawn(chat_id):
    """Posts one 4-option trivia round in chat_id. First correct tap wins a random 💰
    TRIVIA_CCM_MIN-TRIVIA_CCM_MAX ccm; a wrong tap loses the same random roll instead (floored
    at the tapper's current balance, never negative). Two-stage post — question alone first,
    options+buttons TRIVIA_OPTIONS_REVEAL_DELAY seconds later — same anti-mash-tap technique
    the rarity-gate quiz's spawn message doesn't need (that one's answers are hidden behind a
    genuine gate, not a race against the clock)."""
    if chat_id in pending_trivia_quiz:
        return
    try:
        pool = await _get_trivia_pool()
        if not pool:
            return  # nothing added via /addtrivia yet — skip this cycle quietly
        q = random.choice(pool)
        options = q["options"][:]
        ...
        stage1_text = (
            f"🧠 <b>Trivia Time!</b>\n\n"
            f"❓ <b>{escape_html(q['question'])}</b>\n\n"
            f"👀 <i>Read it first — options revealing in {TRIVIA_OPTIONS_REVEAL_DELAY}s...</i>"
        )
        options_text = "\n".join(f"<b>{i + 1}.</b> {escape_html(opt)}" for i, opt in enumerate(options))
        stage2_text = (
            f"🧠 <b>Trivia Time!</b>\n\n"
            f"❓ <b>{escape_html(q['question'])}</b>\n\n"
            f"{options_text}\n\n"
            f"⏱ <b>{TRIVIA_TIMEOUT_SECONDS}s</b> — first correct tap wins 💰 <code>{TRIVIA_CCM_MIN}-{TRIVIA_CCM_MAX}</code> ccm!\n"
            f"⚠️ <i>Wrong tap costs ccm too — one try each, no second chances.</i>"
        )
        ...
        # Set immediately (before the reveal delay), same reasoning as the rarity-gate quiz:
        # the "one trivia at a time per chat" guard above also has to cover this window, since
        # nothing's tappable yet anyway — "revealed": False is belt-and-suspenders on top of that.
        quiz_state = {
            "options": options,
            "correct_index": correct_index,
            "question": q["question"],
        ...
        try:
            ...
            pass
        # Answering window starts at reveal, not at the original post — the reading pause never
        # eats into anyone's real answering time.
...
# (character spawns): separate counter, separate random target, so the two features never
# compete for or interfere with each other's pacing. Skips quietly while a character spawn or
# rarity-gate quiz is already up in that chat, purely so the group isn't juggling two
# competing "tap this" prompts at once — trivia just waits for the next message after that
# clears instead of queuing anything.
@bot1.on(events.NewMessage(incoming=True))
...
async def trivia_answer_callback(event):
    ...
    if not quiz or quiz.get("solved") or not quiz.get("revealed"):
        return await event.answer("⌛ This trivia round is already over.", alert=True)
    if time.time() - quiz["quiz_time"] > TRIVIA_TIMEOUT_SECONDS:
        return await event.answer("⌛ Time's up.", alert=True)
    # 🔒 One attempt per person, no retries — check-and-mark with no await in between, same
    # pattern as the rarity-gate quiz above.
    attempted_users = quiz.setdefault("attempted_users", set())
    if event.sender_id in attempted_users:
        return await event.answer("⚠️ You already used your one try.", alert=True)
    attempted_users.add(event.sender_id)

    if chosen_idx != quiz["correct_index"]:
        # 💰 Wrong tap costs ccm too — same TRIVIA_CCM_MIN-MAX roll as a correct answer would
        # have WON, just subtracted instead. Never pushes the balance negative: deducts at most
        # whatever the tapper currently has. trivia_wrong (alongside trivia_correct on the win
        # path below) is what /triviatop's "total answerers" count is built from.
        penalty_roll = random.randint(TRIVIA_CCM_MIN, TRIVIA_CCM_MAX)
        actual_deduct = 0
        try:
            wrong_user = await users_catcher_col.find_one({"user_id": event.sender_id})
            current_balance = (wrong_user or {}).get("ccm_balance", 0)
            actual_deduct = min(current_balance, penalty_roll)
            update_ops = {"$inc": {"trivia_wrong": 1}}
            if actual_deduct > 0:
                update_ops["$inc"]["ccm_balance"] = -actual_deduct
            await users_catcher_col.update_one({"user_id": event.sender_id}, update_ops, upsert=True)
        except Exception as e:
            print(f"Trivia wrong-answer ccm penalty error: {e}")
        return await event.answer(
            f"❌ Wrong! −{actual_deduct:,} 💰 ccm. No second chances.",
            alert=True
        )
    ...
    async with spawn_locks[chat_id]:
        quiz = pending_trivia_quiz.get(chat_id)
        if not quiz or quiz.get("solved"):
            return await event.answer("⌛ Someone else already got it.", alert=True)
        quiz["solved"] = True
        winner_id = event.sender_id
    mention = await get_html_mention(event, winner_id)

    reward = random.randint(TRIVIA_CCM_MIN, TRIVIA_CCM_MAX)
    try:
        await users_catcher_col.update_one(
            {"user_id": winner_id},
            {"$inc": {"ccm_balance": reward, "trivia_correct": 1}},
            upsert=True
        )
    except Exception as e:
        print(f"Trivia reward ccm error: {e}")

    try:
        await event.edit(
            f"🏆 <b>{mention} got it first!</b>\n"
            f"✔ <b>Correct answer:</b> {escape_html(quiz['options'][quiz['correct_index']])}\n"
            f"💰 <b>Reward:</b> +<code>{reward:,}</code> ccm",
            parse_mode='html',
            buttons=[[Button.inline("🏆 Global Top 50", data="trivialb")]]
        )
    except Exception:
        pass
    await event.answer(f"✅ Correct! +{reward:,} 💰 ccm", alert=True)
...
async def trivia_quiz_timeout_watcher(chat_id, msg_id):
    try:
        ...
        # 🛡️ RELIABILITY: a leftover pending_trivia_quiz entry silently blocks every future
        # trivia round in this chat (see the guard at the top of trivia_message_counter_handler)
        # — never leave one stuck behind, no matter what goes wrong above.
        print(f"Trivia Timeout Watcher Error: {e}")
        if pending_trivia_quiz.get(chat_id, {}).get("msg_id") == msg_id:
            del pending_trivia_quiz[chat_id]

# ---- /addtrivia: owner-only, step-by-step authoring of a trivia question. DM-only so nobody
# in the group sees the answer being typed. Same wizard shape as /addquiz above. ----
@bot1.on(events.NewMessage(pattern=own_pattern(r'^[/.]addtrivia(?:@\w+)?$', 'bot1')))
    ...
    try:
        async with bot1.conversation(event.chat_id, timeout=300) as conv:
            ...
            if correct_letter == "/CANCEL":
                return await conv.send_message("❌ Cancelled.")
            if correct_letter not in option_labels:
                return await conv.send_message("❌ That's not A/B/C/D. Cancelled — run /addtrivia again.")
            correct_index = option_labels.index(correct_letter)
            ...
            await trivia_bank_col.insert_one(trivia_doc)
            ...
            await conv.send_message(preview, parse_mode='html')
            await conv.send_message(
                f"✅ <b>Saved!</b> This will now show up in the trivia rotation — 💰 "
                f"<code>{TRIVIA_CCM_MIN}-{TRIVIA_CCM_MAX}</code> ccm win/loss either way.",
                parse_mode='html'
            )
    except asyncio.TimeoutError:
...
def prewarm_media_identity_cache(stored_msg, char_id):
    ...
    ⚠️ In-memory only, like _MEDIA_ID_CACHE itself — a bot restart clears this back to reactive-
    only warming until each file gets looked up again naturally (still bounded by
    MEDIA_IDENTIFY_CACHE_TTL, 7 days). Ask if you want this backed by a persistent collection
    instead so it survives restarts too."""
    try:
...
async def warm_media_identity_cache_from_db(progress_status=None):
    """Backfills _MEDIA_ID_CACHE for the WHOLE existing roster — prewarm_media_identity_cache
    above only covers imports going forward, so anything imported before this fix existed (and
    everything again after every bot restart, since the cache is in-memory) still starts cold.
    Batch-fetches up to 200 messages per Telegram API call (get_messages' own limit) instead of
    one call per character — for a roster in the thousands, a handful of API calls instead of
    thousands of them. Safe to re-run any time (e.g. via /warmmediacache after a big import
    batch) — just re-sets the same cache entries.

    🩹 FIX (owner report — only ever warms exactly one batch's worth, e.g. 200/7000, no matter
    the roster size): this used to fire every batch's get_messages back-to-back with zero
    pacing, and treated ANY exception — including a routine FloodWaitError — as "skip this
    batch forever", so the very first flood wait (near-inevitable once you're making dozens of
    these calls in a tight loop) silently doomed every batch after it, landing exactly on one
    batch's worth warmed. Same class of bug rehashall's own get_messages call already had to
    fix (see its "gentle pacing — avoids FloodWaitError" comment) — applying the same two-part
    fix here: a small delay between batches to avoid tripping flood control in the first place,
    and actually waiting out + retrying a FloodWaitError instead of dropping the batch."""
    docs = await characters_base_col.find({}, {"char_id": 1, "storage_msg_id": 1, "_id": 0}).to_list(length=None)
    id_to_char = {d["storage_msg_id"]: d["char_id"] for d in docs if d.get("storage_msg_id")}
    ...
    for batch_num, i in enumerate(range(0, len(all_ids), 200), start=1):
        ...
        for attempt in range(5):  # absorbs normal flood waits; caps out so a persistent, non-flood failure still moves on instead of looping forever
            try:
                ...
                print(f"⚠️ warm_media_identity_cache_from_db: batch {batch_num}/{total_batches} fetch failed: {e}")
                break
        if messages:
            for m in messages:
                if not m:
    ...
    print(f"🚀 [media cache] Pre-warmed {warmed}/{len(all_ids)} characters' file identities — /who on a repost of any of them now resolves as a plain dict lookup, no download needed.")
...
async def warm_media_cache_command(event):
    """Manual re-trigger for warm_media_identity_cache_from_db — mainly useful right after a
    big /syncfromcatch or /xbotimportall run, so the whole batch is instantly recognizable
    without waiting for the automatic startup warm-up (which only runs once, at boot)."""
    if event.sender_id != OWNER_ID: return
    status = await event.reply("⏳ Warming /who's file-identity cache from the whole roster...")
    warmed, total = await warm_media_identity_cache_from_db(progress_status=status)
    await status.edit(
        f"✅ Warmed {warmed}/{total} characters. Reposts of their stored media now resolve "
        f"instantly via /who — no download or hashing needed.\n"
        f"⚠️ In-memory only (like the cache itself) — clears on restart, but the next boot "
        f"re-runs this automatically."
    )
                    ...
                    buttons = types.ReplyInlineMarkup(rows=[
                        types.KeyboardButtonRow(buttons=[
                            types.KeyboardButtonCopy(
                                ...
                                copy_text=f"/fuck {char_name}{CATCH_REQUIRED_SUFFIX}"
                            ),
                            types.KeyboardButtonCopy(
                                text="/catch",
                                copy_text=f"/catch {char_name}{CATCH_REQUIRED_SUFFIX}"
                            )
                        ])
                    ])
                    await event.reply(reveal_text, parse_mode='html', buttons=buttons)
...
async def sync_from_catch_command(event):
    ...
    if explicit_start:
        start_id = max(CATCHBOT_SYNC_MIN_ID, min(int(explicit_start), CATCHBOT_SYNC_MAX_ID))
        ...
        if state and state.get("active") and state.get("last_checked_id"):
            # 🩹 Clamped on BOTH ends — the owner may have narrowed or moved the range with
            # /syncfromcatchrange since this checkpoint was saved (e.g. raised the min past
            # where an old run left off, or lowered the max below it).
            start_id = max(CATCHBOT_SYNC_MIN_ID, min(state["last_checked_id"] + 1, CATCHBOT_SYNC_MAX_ID))
        else:
    ...
    status_msg = await event.reply(
        ...
        f"hours for the full {CATCHBOT_SYNC_MIN_ID}-{CATCHBOT_SYNC_MAX_ID} range). If the bot "
        f"restarts, it'll resume from here automatically next time it boots, or whenever you "
        f"run /syncfromcatch again. Use /syncfromcatchrange to change this window.",
        parse_mode='html'
    )
...
async def sync_from_catch_status_command(event):
    ...
    await event.reply(
        ...
        f"Status: {running}\n"
        f"🎯 Range: <code>{CATCHBOT_SYNC_MIN_ID}-{CATCHBOT_SYNC_MAX_ID}</code>\n"
...
async def sync_from_catch_range_command(event):
    """Lets the owner move /syncfromcatch's scan window (min & max id) without editing code —
    persisted to bot_settings_col (see load_catchbot_sync_range_cache, loaded at boot) so it
    survives a restart. Bare /syncfromcatchrange (no numbers) just reports the current window."""
    global CATCHBOT_SYNC_MIN_ID, CATCHBOT_SYNC_MAX_ID
    if event.sender_id != OWNER_ID:
        return
...
async def load_and_start_monitor_userbot():
    ...
    try:
        ...
        if sync_state and sync_state.get("active"):
            resume_from = max(CATCHBOT_SYNC_MIN_ID, min(sync_state.get("last_checked_id", 0) + 1, CATCHBOT_SYNC_MAX_ID))
...
async def send_paginated_harem(client, chat_id, user_id, page=1, edit_msg_id=None, viewer_id=None, fav_idx=0):
    ...
    if is_unlimited_vault:
        ...
        if rarity_filter:
            char_event_map = {}
            if event_filter:
                char_event_map = {c["char_id"]: (c.get("event") or "General") for c in await get_all_characters_cached()}
            for item in raw_harem:
                if isinstance(item, dict) and classify_rarity(item.get("rarity")) == classify_rarity(rarity_filter):
                    if event_filter and char_event_map.get(item.get("char_id")) != event_filter:
                        continue
...
# 🧠 GLOBAL TRIVIA TOP 50 — paginated inline-button leaderboard, 10 rows/page (5 pages),
# ◀ Previous / Next ▶ nav, same fetch-once-then-slice shape as main_pyh.py's
# render_trivia_leaderboard_page. Ranked by trivia_correct (see trivia_answer_callback near
# perform_catch) instead of trivia_points/rank — there's no points/rank ladder here, just a
# flat correct-answer count. Reachable via /triviatop OR the "🏆 Global Top 50" button that
# now shows on every trivia win (see trivia_answer_callback) — both open page 0 fresh.
TRIVIA_LB_PAGE_SIZE = 10
TRIVIA_LB_TOTAL = 50

async def render_trivia_leaderboard_page(page):
    """Builds (text, buttons) for one page of the Global Top 50 Trivia leaderboard. page is
    0-indexed: 0 = ranks 1-10, ..., 4 = ranks 41-50. Re-fetches the top 50 fresh on every call
    (cheap — one small, limited Mongo query) so a page tap never shows stale standings from
    whenever an earlier page first loaded."""
    top_users = await users_catcher_col.find(
        {"trivia_correct": {"$gt": 0}}
    ).sort("trivia_correct", -1).limit(TRIVIA_LB_TOTAL).to_list(length=TRIVIA_LB_TOTAL)
    total = len(top_users)
    ...
    if not page_rows:
        ...
        mentions = await _resolve_top_mentions(bot1, page_rows)
        lines = []
        for i, (u, mention) in enumerate(zip(page_rows, mentions)):
            rank = start + i + 1
            rank_tag = TOP_MEDALS.get(rank - 1, f"<code>#{rank}</code>")
            count = u.get("trivia_correct", 0)
            lines.append(f"{rank_tag}  {mention} — <code>{count:,} correct</code>")
        body = "\n".join(lines)
    text = "🧠 <b>GLOBAL TRIVIA TOP 50</b>\n\n" + body

    # 🩹 Purely-informational "everyone who's ever answered" count — a broader number than just
    # the 50 shown above, since it also counts people who've only ever answered WRONG so far.
    total_participants = await users_catcher_col.count_documents(
        {"$or": [{"trivia_correct": {"$gt": 0}}, {"trivia_wrong": {"$gt": 0}}]}
    )
    ...
    if nav_row:
        buttons.append(nav_row)
    buttons.append([Button.inline(f"👥 Total answerers: {total_participants:,}", data="trivia_noop")])
    return text, buttons
...
async def trivia_leaderboard_open_callback(event):
    """Entry point for the "🏆 Global Top 50" button shown on a trivia win — always opens
    fresh at page 0, same as typing /triviatop."""
    text, buttons = await render_trivia_leaderboard_page(0)
    try:
        await event.edit(text, parse_mode='html', buttons=buttons)
        await event.answer()
...
async def trivia_leaderboard_page_callback(event):
    """◀ Previous / Next ▶ taps — edits the same message in place to the requested page."""
    page = int(event.pattern_match.group(1))
...
@bot1.on(events.CallbackQuery(pattern=r'^trivia_noop$'))
async def trivia_noop_callback(event):
    await event.answer()  # purely informational row — just acks the tap, does nothing
...
# ==========================================
# 🏪 /market — opens the Card Market Mini App (see /webapp in the project root: a separate
# FastAPI process implementing Buy/Sell/Bid-auction/Scrap, sharing this bot's own MongoDB and
# ccm_balance/harem data — /webapp/README.md has full deployment steps). This button is the
# ONLY integration point needed on the bot side; the Mini App's backend talks to the same
# database directly for everything else.
# 🩹 CHANGED: card trading (/trade, /buy, /sell, /market) was previously removed from this bot
# entirely — this brings equivalent functionality back as a Web App instead of chat commands.
# ==========================================
MARKET_WEBAPP_URL = os.environ.get("MARKET_WEBAPP_URL")  # e.g. https://your-market.onrender.com — see /webapp/README.md

@bot1.on(events.NewMessage(pattern=own_pattern(r'^[/.]market(?:@\w+)?$', 'bot1')))
async def market_launcher_handler(event):
    if not MARKET_WEBAPP_URL:
        return await event.reply(
            "⚠️ <b>Market ကို ဒီ bot ထဲမှာ ချိတ်ဆက်ရသေးပါ။</b> Owner အတွက်: MARKET_WEBAPP_URL "
            "environment variable ကို webapp ရဲ့ deployed HTTPS URL နဲ့ set ပါ (/webapp/README.md ကြည့်ပါ)။",
            parse_mode='html'
        )
    await event.reply(
        "🏪 <b>Card Market</b>\n\nဝယ်၊ ရောင်း၊ လေလံတင်၊ (.) 24 နာရီ auction၊ Scrap (ကဒ်မီးရှို့ ccm ယူနိုင်တဲ့) စတာတွေကို "
        "အောက်က button ကနေ ဝင်ကြည့်ပါ။",
        parse_mode='html',
        buttons=[[Button.web_app("🏪 Open Market", MARKET_WEBAPP_URL)]]
    )
...
# (/market above is the card-trading equivalent now — a Web App, not a chat command, so
# there's nothing analogous to restrict here; /gift is still the only IN-CHAT transfer command.)
# ==========================================
...
async def start_system():
    ...
    asyncio.create_task(warm_media_identity_cache_from_db())
