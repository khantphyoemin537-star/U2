...
async def get_all_events_cached():
    """Distinct non-'General' 'event' values across the whole roster (e.g. "💞Valentine💞",
    "🎃Halloween🎃") — powers /hmode's event picker. Sorted for a stable, deterministic order
    so a button's index-into-this-list (used as its callback data, since a full event string
    with emoji can be awkward/long for callback_data) keeps meaning the same character between
    the picker being rendered and the tap being handled, as long as the roster's event set
    hasn't changed in between. Same cache-then-invalidate-on-roster-change pattern as
    get_all_categories_cached above."""
    try:
        cached = await redis_client.get("cache:chars:events")
        if cached:
            return json.loads(cached)
    except Exception as e:
        print(f"⚠️ Redis get_all_events_cached error: {e}")
    raw = await characters_base_col.distinct("event")
    data = sorted({str(e).strip() for e in raw if e and str(e).strip() and str(e).strip().lower() != "general"})
    try:
        await redis_client.setex("cache:chars:events", CHAR_CACHE_TTL, json.dumps(data, default=str))
    except Exception as e:
        print(f"events cache write error: {e}")
    return data
...
async def invalidate_character_caches():
    ...
    try:
        await redis_client.delete("cache:chars:all", "cache:chars:categories", "cache:chars:category_totals", "cache:chars:events")
    except Exception as e:
...
FORCE_SUB_CHAT_ID = int(os.environ.get("FORCE_SUB_CHAT_ID", "-1003948400788"))
FORCE_SUB_REWARD_USD = 500 / MMK_PER_USD
FORCE_SUB_MEMBERSHIP_TTL = 1800  # 🚀 PERF FIX (per owner report — /who felt slow): was 120s.
# is_force_sub_member() below is called by force_sub_gate on EVERY gated command — including
# .who/.w/.waifu — and a cache miss means a live bot1.get_permissions() Telegram API round trip
# BEFORE the actual command handler even starts. At 120s, any player who hadn't touched a
# gated command in the last 2 minutes paid that round trip again on their very next /who. Now
# that force_sub_join_tracker (below) also reacts to LEAVES in real time and invalidates the
# cache immediately when someone actually leaves, a much longer TTL is safe — a member who
# hasn't left still reads back "yes" instantly from cache, and someone who does leave is caught
# right away by the leave event instead of waiting out the TTL.
...
FORCE_SUB_GATED_COMMANDS = {
    ...
    "hmode",                   # rarity filter
    "gift",
    "fuck", "morgan",          # kept from the original set
}
...
# ---- JOIN REWARD — REINSTATED (per owner request), new shape: instead of the old flat
# +FORCE_SUB_REWARD_USD + one random card, a new FORCE_SUB_CHAT_ID member now gets ONE random
# card from EVERY rarity tier (see RARITY_TIERS — 9 tiers today, so up to 9 cards; fewer only
# if a tier genuinely has no characters in it yet). Granted once per user — guarded by the
# same "force_sub_rewarded" flag every user doc already has (see ensure_user_registered) — so
# leaving and rejoining can't be farmed for repeat grants. FORCE_SUB_REWARD_USD above is no
# longer used by this path (kept in case a currency bonus gets layered back in later); harem
# entries this grants are still tagged source="force_sub_reward" for the same reason the old
# ones were, in case the owner ever wants to find/reverse them in bulk.
async def _grant_force_sub_join_reward(user_id, fullname):
    """Grants one random card from EACH rarity tier to a first-time FORCE_SUB_CHAT_ID joiner,
    then quietly DMs them the list. Deliberately no group announcement — welcome/goodbye
    messages were turned off for this group on purpose (see welcome_goodbye above), so this
    reward never posts anything into the group itself."""
    try:
        await ensure_user_registered(user_id, fullname)
        user_doc = await users_catcher_col.find_one({"user_id": user_id}, {"force_sub_rewarded": 1})
        if user_doc and user_doc.get("force_sub_rewarded"):
            return
        characters_list = await get_all_characters_cached()
        if not characters_list:
            return
        by_tier = {tier: [] for tier in RARITY_TIERS}
        for char in characters_list:
            tier = char.get("rarity_tier") or classify_rarity(char.get("rarity", ""))
            if tier in by_tier:
                by_tier[tier].append(char)
        now = time.time()
        granted, harem_entries = [], []
        for tier in RARITY_TIERS:
            pool = by_tier.get(tier)
            if not pool:
                continue  # no character exists in this tier yet — skip it, don't fail the grant
            chosen = random.choice(pool)
            granted.append(chosen)
            harem_entries.append({
                "char_id": chosen["char_id"],
                "caught_date": now,
                "rarity": chosen.get("rarity", tier),
                "status": "vault",
                "source": "force_sub_reward"
            })
        if not harem_entries:
            return
        # 🩹 force_sub_rewarded flips to True in this SAME update as the harem push, so two
        # join events racing for the same user (e.g. a flaky reconnect re-delivering the
        # ChatAction) can't both slip past the check above and double-grant.
        await users_catcher_col.update_one(
            {"user_id": user_id},
            {
                "$push": {"harem": {"$each": harem_entries}},
                "$inc": {"total_caught": len(harem_entries)},
                "$set": {"force_sub_rewarded": True, "fullname": fullname}
            }
        )
        try:
            lines = ["🎉 <b>Thanks for joining!</b> Here's one card from every rarity, on the house:", ""]
            for char in granted:
                tier = char.get("rarity_tier") or classify_rarity(char.get("rarity", ""))
                emoji = RARITY_EMOJI.get(tier, RARITY_DEFAULT_EMOJI)
                lines.append(f"{emoji} {escape_html(char.get('name', '?'))}")
            lines.append("")
            lines.append("Check /harem to see them all. 🤍")
            await send_safe_message(bot1, user_id, "\n".join(lines), parse_mode='html')
        except Exception as dm_error:
            # Most common cause: the user has never opened a DM with bot1, so Telegram won't
            # let it message them first — not a real failure, the cards are already granted.
            print(f"force_sub_join_reward DM failed for {user_id}: {dm_error}")
    except Exception as e:
        await report_system_error("_grant_force_sub_join_reward", f"user_id={user_id}: {e}")

@bot1.on(events.ChatAction(chats=FORCE_SUB_CHAT_ID))
async def force_sub_join_tracker(event):
    """Keeps force_sub_membership_cache accurate for FORCE_SUB_CHAT_ID in real time:
    - on join: marks the user a member (so is_force_sub_member() doesn't need a fresh API
      call right after) and kicks off the 9-tier join reward above.
    - on leave/kick: marks the user a NON-member immediately. This is what makes it safe to
      run FORCE_SUB_MEMBERSHIP_TTL at 30 minutes instead of 2 (see that constant above) without
      force_sub_gate ever going stale for someone who actually left."""
    ...
    if event.user_joined or event.user_added:
        ...
        try:
            ...
            fullname = f"{first_name} {last_name}".strip()
            if not fullname:
                fullname = getattr(user_entity, 'username', '') or f"User {user_id}"
        except Exception:
            fullname = f"User {user_id}"
        asyncio.create_task(_grant_force_sub_join_reward(user_id, fullname))
    elif event.user_left or event.user_kicked:
        force_sub_membership_cache[user_id] = (False, time.time() + FORCE_SUB_MEMBERSHIP_TTL)
...
async def system_callback_router(event):
    ...
    if data == "nav_back_home":
        ...
        await hmode_entry_handler(event)
...
# 🎁 FORCE-SUB GROUP AUTO-REVEAL (bot1's own spawns) — see release_spawn below, which fires
# this right after a spawn goes live in FORCE_SUB_CHAT_ID. Posts the exact same reveal /who
# would (name, rarity, artist, the /fuck [name] command + copy button) automatically, so
# members of that one group never have to type .w/.who/.waifu at all. Guarded by
# spawn_data["revealed"] both ways: who_reveal_handler's Path A sets it True the instant a
# real /who beats this to it, and this function sets it True itself before sending, so the two
# can never both post a reveal for the same spawn.
async def _auto_reveal_force_sub_spawn(chat_id, spawn_msg_id):
    spawn_data = active_group_spawns.get(chat_id)
    if not spawn_data or spawn_data.get("spawn_msg_id") != spawn_msg_id or spawn_data.get("revealed"):
        return
    ...
    try:
        ...
        reveal_lines = ["◈ found me, huh? 🎁 (free reveal — force-sub perk)", "", name_line]
        ...
        try:
            ...
            await bot1.send_message(chat_id, reveal_text, parse_mode='html', reply_to=spawn_msg_id, buttons=buttons)
        except Exception as button_error:
            await report_system_error("force_sub_auto_reveal_button_fallback", f"Button error: {button_error}")
            await bot1.send_message(chat_id, reveal_text, parse_mode='html', reply_to=spawn_msg_id)
    except Exception as e:
        await report_system_error("_auto_reveal_force_sub_spawn", f"chat {chat_id}, spawn {spawn_msg_id}: {e}")
...
async def release_spawn(chat_id, chosen_char):
    ...
    try:
        ...
        # 🎁 FORCE-SUB GROUP PRIVILEGE (per owner request): members of FORCE_SUB_CHAT_ID don't
        # need to type .w/.who/.waifu at all — the name posts automatically right after the
        # spawn. /fuck [name] still works exactly the same either way. Backgrounded via
        # create_task so it can never add latency to the spawn itself.
        if chat_id == FORCE_SUB_CHAT_ID:
            asyncio.create_task(_auto_reveal_force_sub_spawn(chat_id, spawn_msg.id))
        return True
...
async def _identify_media_and_reply(event, media_msg, known_source_bot=None):
    ...
    if best_match:
        ...
        try:
            ...
            # 🩹 FIX (per owner report — "fuck AND catch buttons both show up"): this used to
            # ALWAYS print /fuck (button) + /catch (text+button) together, no matter where the
            # photo/video actually came from. bot1's own roster and catch_bot's roster share
            # IDENTICAL artwork for every auto-imported character (see
            # auto_import_character_from_catchbot), so an own-DB phash hit here can't by itself
            # say which bot actually POSTED this exact message — so both commands got printed
            # unconditionally, every time, even when the caller already knew better.
            #
            # known_source_bot already tells us the real answer whenever it's available — it's
            # only ever set (see who_reveal_handler above) when this /who is a reply to a
            # message from a bot we've explicitly mapped via /xbotsetbot, i.e. we KNOW for a
            # fact this specific photo/video is catch_bot's (or another mapped bot's), not
            # bot1's. So: known_source_bot set -> show ONLY that bot's own command, same
            # single-button pattern _xbot_identify_fallback_and_reply and
            # force_sub_group_xbot_auto_reveal below already use. known_source_bot unset (a
            # bare /who-as-caption, or a reply to a regular user's saved photo — genuinely no
            # evidence which bot it's from) -> fall back to bot1's own /fuck, the one thing an
            # own-roster hit DOES actually tell us.
            if known_source_bot:
                xbot_label = XBOT_SOURCE_LABELS.get(known_source_bot, (known_source_bot.replace("_", " ").title(), None))
                shown_command = build_xbot_catch_command(best_match['name'], known_source_bot)
                command_line = f"<code>{escape_html(shown_command)}</code>" if shown_command else f"🌐 <b>Bot:</b> {escape_html(xbot_label[0])}"
                button_list = (
                    [types.KeyboardButtonCopy(text=f"🤍 {xbot_label[1]}", copy_text=shown_command)]
                    if shown_command else []
                )
            else:
                shown_command = f"/fuck {best_match['name']}"
                command_line = f"<code>{escape_html(shown_command)}</code>"
                button_list = [types.KeyboardButtonCopy(text="🤍 /fuck", copy_text=shown_command)]
            ...
            try:
                if button_list:
                    buttons = types.ReplyInlineMarkup(rows=[types.KeyboardButtonRow(buttons=button_list)])
                ...
                await event.reply(reveal_text + f"\n\n{command_line}", parse_mode='html')
        except Exception as e:
            print(f"identify reply error: {e}")
        return

    # ✅ STEP 2: ကိုယ်ပိုင် DB မှာ မတွေ့ရင်၊ ပြီးမှ cross-bot lookup ကို စစ်ပါ (but only if we know which bot it is)
    if known_source_bot:
        return await _xbot_identify_fallback_and_reply(event, media_msg, known_source_bot=known_source_bot)

    # ✅ STEP 3: ဘယ်မှာမှ မတွေ့ရင် "This one isn't recognized" ပြန်ပါ
...
async def who_reveal_handler(event):
    ...
    # 🚀 PERF FIX (per owner report — "why does /who take so long?"): Path A below, replying
    # to the group's CURRENT live spawn, is by far the most common case — every /fuck attempt
    # starts with a /who first — and needs NOTHING beyond the synchronous event.reply_to_msg_id
    # (already delivered with the update, zero network cost) to know it applies. It used to run
    # AFTER the known_source_bot block further down, which does an unconditional
    # `await event.get_reply_message()` (a real Telegram API round trip) on every reply-based
    # /who — a whole extra network hop tacked onto the hottest path in the game, for a check
    # Path A never even reads. Checking Path A's cheap condition first and branching straight
    # into it lets the common case skip that fetch entirely. Path B (identify a saved/reposted
    # photo, which DOES need known_source_bot) still runs the exact same fetch+bot-check as
    # before — just now only for the cases that actually need it.
    if not event.is_private and chat_id in active_group_spawns:
        ...
        if event.is_reply and event.reply_to_msg_id == spawn_data["spawn_msg_id"]:
            ...
            spawn_data["revealed"] = True  # 🎁 tells the FORCE_SUB_CHAT_ID auto-reveal task (see
            # _auto_reveal_force_sub_spawn) a real /who already covered this spawn, so it doesn't
            # also post its own reveal on top of this one.
            try:
                char_name = spawn_data['name']
                rarity_tier = classify_rarity(spawn_data.get('rarity', ''))
                rarity_emoji = RARITY_EMOJI.get(rarity_tier, RARITY_DEFAULT_EMOJI)
    ...
    # 🩹 Only reached now when Path A above didn't match (see the PERF FIX comment up top) —
    # a not-yet-active-spawn reply, a reply in a chat with no live spawn, or a non-reply /who.
...
# 🎁 FORCE-SUB GROUP AUTO-REVEAL (OTHER bots' spawns) — same free-reveal privilege as
# _auto_reveal_force_sub_spawn above (bot1's own spawns), but for a DIFFERENT collector bot
# (catch_bot, obtain_bot, ...) that ALSO spawns characters directly into FORCE_SUB_CHAT_ID.
# The instant one of THEIR spawns lands there, bot1 recognizes it via the same cross-bot hash
# lookup /who's Path B already uses (get_cached_xbot_lookup) and posts the name right away —
# no .w/.who/.waifu needed. Only reacts to messages from a bot mapped via /xbotsetbot; stays
# completely silent (no "not recognized" spam) for anything else — including a genuine miss,
# since unlike the on-demand /who flow nobody actually asked here, so there's nothing to answer.
@bot1.on(events.NewMessage(chats=FORCE_SUB_CHAT_ID, incoming=True,
                            func=lambda e: (e.photo or e.video) and not e.sticker))
async def force_sub_group_xbot_auto_reveal(event):
    known_source_bot = await get_bot_mapping(event.sender_id)
    if not known_source_bot:
        return  # not a bot we've mapped via /xbotsetbot — leave it alone (could just be a
        # regular user's photo/video shared in the group)
    try:
        ...
        doc = await get_cached_xbot_lookup(hash_val, source_bot_filter=known_source_bot)
        if not doc:
            return  # not in the cross-bot hash database yet — stay silent, don't guess
        name = doc.get("name") or "?"
        label = XBOT_SOURCE_LABELS.get(known_source_bot, (known_source_bot.replace("_", " ").title(), None))
        final_command = build_xbot_catch_command(name, known_source_bot)
        reveal_text = (
            f"🔎 <b>Recognized!</b> 🎁 <i>(free reveal — force-sub perk)</i>\n\n"
            f"<b>Name:</b> <b>{escape_html(name)}</b>\n"
            f"🌐 <b>Bot:</b> {escape_html(label[0])}\n\n"
        )
        if final_command:
        ...
        try:
            ...
            await report_system_error("force_sub_xbot_auto_reveal_button_fallback", f"Button error: {button_error}")
            await event.reply(reveal_text, parse_mode='html')
    except Exception as e:
        await report_system_error("force_sub_group_xbot_auto_reveal", str(e))
...
# ==========================================
CATCH_BOT_ID = 6157455819  # catch_bot's own Telegram user id — the ".check <id>" DM target
...
# ==========================================
# 🎛 HMODE — HAREM INTERFACE + SORT/FILTER WIZARD
# ==========================================
# 🎨 REDESIGN (per owner request, modeled on a reference bot's /chmode flow — screenshots
# supplied by the owner): /hmode → pick Default/Detailed interface (Screen A). Once a style is
# already set, running /hmode again jumps straight to "already set to X, still choose how to
# sort" (Screen B) instead of re-showing the style picker. From there: Rarity/Event walks
# through picking a preferred RARITY (or Skip) then an EVENT (or Skip, paginated with
# Next/Previous — the roster's event list is open-ended, unlike the fixed 9 rarities) — while
# Sort By Anime just clears any rarity/event filter, since /harem already groups everything by
# anime/category on its own. rarity_filter and event_filter both genuinely RESTRICT /harem's
# listing now (see send_paginated_harem below), not just sort it first — that's the actual
# owner ask ("haremလုပ်ရင် သက်ဆိုင်ရာပဲ ပြနိုင်အောင်").
# IMPROVEMENTS over the reference: a 🔄 Switch Interface button (the reference had no way back
# to Default once you were on Detailed, short of a full Reset), a 🔙 Back on every step instead
# of only Close, and real Previous (not just Next) pagination on the event picker.
# ==========================================
def _hmode_style_label(style):
    return "🦖 Detailed" if style == "detailed" else "🐉 Default"

def _hmode_main_screen(style):
    return (
        f"🎛 <b>YOU CAN CHANGE YOUR HAREM INTERFACE USING THESE BUTTONS</b>\n\n"
        f"Current: <b>{_hmode_style_label(style or 'default')}</b>"
    )

def _hmode_main_buttons(user_id):
    return [
        [
            Button.inline("🐉 Default", data=f"hm_style_default_{user_id}"),
            Button.inline("🦖 Detailed", data=f"hm_style_detailed_{user_id}")
        ],
        [Button.inline("🦕 Reset Preference", data=f"hm_reset_{user_id}")],
    ]

def _hmode_sort_screen(style, user_id):
    text = (
        f"✅ <b>Your Harem Interface Is Already Set To {_hmode_style_label(style)}</b>\n\n"
        f"Still You Can Choose How To Sort Your Harem:"
    )
    buttons = [
        [
            Button.inline("🌸 Rarity / Event", data=f"hm_sort_rarevent_{user_id}"),
            Button.inline("📘 Sort By Anime", data=f"hm_sort_anime_{user_id}")
        ],
        [Button.inline("🔄 Switch Interface", data=f"hm_back_main_{user_id}")],
        [Button.inline("🚮 Close", data=f"hm_close_{user_id}")]
    ]
    return text, buttons

def _hmode_rarity_screen(user_id, current_rarity_filter):
    text = (
        f"❄️ <b>CHOOSE YOUR PREFERRED RARITY</b>\n"
        f"Current: <b>{escape_html(current_rarity_filter) if current_rarity_filter else 'Any'}</b>"
    )
    buttons = []
    for tier in RARITY_TIERS:
        num = RARITY_TIER_TO_NUM[tier]
        full_name = RARITY_NUM_MAP[num]["name"]
        prefix = "✅ " if current_rarity_filter == full_name else ""
        label = f"{prefix}{RARITY_EMOJI[tier]} RARITY: {RARITY_DISPLAY_NAME[tier]}"
        buttons.append([Button.inline(label, data=f"hm_rarity_{num}_{user_id}")])
    buttons.append([Button.inline("⏭ Skip Rarity", data=f"hm_rarity_skip_{user_id}")])
    buttons.append([
        Button.inline("🔙 Back", data=f"hm_back_sort_{user_id}"),
        Button.inline("🚮 Close", data=f"hm_close_{user_id}")
    ])
    return text, buttons

HMODE_EVENTS_PER_PAGE = 10

async def _hmode_event_screen(user_id, current_event_filter, page=1, header_note=""):
    all_events = await get_all_events_cached()
    total = len(all_events)
    total_pages = max(1, (total + HMODE_EVENTS_PER_PAGE - 1) // HMODE_EVENTS_PER_PAGE)
    if page < 1: page = 1
    if page > total_pages: page = total_pages
    start = (page - 1) * HMODE_EVENTS_PER_PAGE
    page_events = all_events[start:start + HMODE_EVENTS_PER_PAGE]
    text = header_note + "🎉 <b>NOW CHOOSE AN EVENT (OR SKIP):</b>"
    if total_pages > 1:
        text += f"\n<i>Page {page}/{total_pages}</i>"
    if not all_events:
        text += "\n\n<i>…no events on the roster yet.</i>"
    buttons = []
    row = []
    for i, ev in enumerate(page_events):
        real_idx = start + i
        is_current = bool(current_event_filter) and current_event_filter.strip().lower() == ev.strip().lower()
        label = f"✅ {ev}" if is_current else ev
        row.append(Button.inline(label, data=f"hm_event_pick_{real_idx}_{user_id}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
    ...
    nav_row = []
    if page > 1:
        nav_row.append(Button.inline("⬅️ Previous", data=f"hm_event_page_{page-1}_{user_id}"))
    if page < total_pages:
        nav_row.append(Button.inline("Next ➡️", data=f"hm_event_page_{page+1}_{user_id}"))
    if nav_row:
        buttons.append(nav_row)
    buttons.append([Button.inline("⏭ Skip Event", data=f"hm_event_skip_{user_id}")])
    buttons.append([
        Button.inline("🔙 Back", data=f"hm_back_rarity_{user_id}"),
        Button.inline("🚮 Close", data=f"hm_close_{user_id}")
    ])
    return text, buttons

async def _hmode_finish(event, user_id):
    user_doc = await users_catcher_col.find_one(
        {"user_id": user_id}, {"harem_style": 1, "rarity_filter": 1, "event_filter": 1, "_id": 0}
    ) or {}
    style = user_doc.get("harem_style") or "default"
    rarity = user_doc.get("rarity_filter")
    ev = user_doc.get("event_filter")
    text = (
        f"✅ <b>Preferences saved!</b>\n\n"
        f"⌁ Interface: {_hmode_style_label(style)}\n"
        f"⌁ Rarity: {escape_html(rarity) if rarity else 'Any'}\n"
        f"⌁ Event: {escape_html(ev) if ev else 'Any'}\n\n"
        f"Use /harem to see your vault."
    )
    try:
...
async def hmode_entry_handler(event):
    user_id = event.sender_id
    user_doc = await users_catcher_col.find_one({"user_id": user_id}, {"harem_style": 1, "_id": 0}) or {}
    style = user_doc.get("harem_style")
    if not style:
        text = _hmode_main_screen(style)
        buttons = _hmode_main_buttons(user_id)
    else:
        text, buttons = _hmode_sort_screen(style, user_id)
    await event.reply(text, buttons=buttons, parse_mode='html')

@bot1.on(events.CallbackQuery(pattern=r'^hm_style_(default|detailed)_(\d+)$'))
async def hmode_style_callback(event):
    new_style = event.pattern_match.group(1)
    if isinstance(new_style, bytes):
        new_style = new_style.decode('utf-8')
    owner_user_id = int(event.pattern_match.group(2))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။ /hmode ကို ကိုယ်တိုင်နှိပ်ပါ။", alert=True)
    await users_catcher_col.update_one(
        {"user_id": owner_user_id}, {"$set": {"harem_style": new_style}}, upsert=True
    )
    await event.answer(f"…interface set to {new_style}")
    text, buttons = _hmode_sort_screen(new_style, owner_user_id)
    try:
        await event.edit(text, parse_mode='html', buttons=buttons)
    except Exception:
        pass

@bot1.on(events.CallbackQuery(pattern=r'^hm_reset_(\d+)$'))
async def hmode_reset_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    await users_catcher_col.update_one(
        {"user_id": owner_user_id},
        {"$set": {"harem_style": None, "rarity_filter": None, "event_filter": None}},
        upsert=True
    )
    await event.answer("…all preferences reset")
    try:
        await event.edit(
            "🦕 <b>All harem preferences have been reset.</b>\nUse /hmode to set them up again.",
            parse_mode='html', buttons=None
        )
    except Exception:
        pass

@bot1.on(events.CallbackQuery(pattern=r'^hm_sort_anime_(\d+)$'))
async def hmode_sort_anime_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    await users_catcher_col.update_one(
        {"user_id": owner_user_id}, {"$set": {"rarity_filter": None, "event_filter": None}}, upsert=True
    )
    await event.answer("…sorted by anime, filters cleared")
    try:
        await event.edit(
            "📘 <b>Sort preference set to By Anime.</b>\n"
            "Any Rarity/Event filter has been cleared.\n\n"
            "Use /harem to see your vault.",
            parse_mode='html', buttons=None
        )
    except Exception:
        pass

@bot1.on(events.CallbackQuery(pattern=r'^hm_sort_rarevent_(\d+)$'))
async def hmode_sort_rarevent_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    user_doc = await users_catcher_col.find_one({"user_id": owner_user_id}, {"rarity_filter": 1, "_id": 0}) or {}
    text, buttons = _hmode_rarity_screen(owner_user_id, user_doc.get("rarity_filter"))
    await event.answer()
    try:
        await event.edit(text, parse_mode='html', buttons=buttons)
    except Exception:
...
@bot1.on(events.CallbackQuery(pattern=r'^hm_rarity_(\d+|skip)_(\d+)$'))
async def hmode_rarity_callback(event):
    ...
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    if choice == "skip":
        await users_catcher_col.update_one({"user_id": owner_user_id}, {"$set": {"rarity_filter": None}}, upsert=True)
        await event.answer("…rarity skipped")
        header_note = "⏭ <b>Rarity skipped.</b>\n\n"
    else:
        rarity_data = RARITY_NUM_MAP.get(choice)
        if not rarity_data:
            return await event.answer("❌ Invalid rarity.", alert=True)
        await users_catcher_col.update_one(
            {"user_id": owner_user_id}, {"$set": {"rarity_filter": rarity_data["name"]}}, upsert=True
        )
        await event.answer("…rarity set")
        header_note = f"✅ <b>Rarity set to {rarity_data['name']}.</b>\n\n"
    user_doc = await users_catcher_col.find_one({"user_id": owner_user_id}, {"event_filter": 1, "_id": 0}) or {}
    text, buttons = await _hmode_event_screen(owner_user_id, user_doc.get("event_filter"), page=1, header_note=header_note)
    try:
        await event.edit(text, parse_mode='html', buttons=buttons)
    except Exception:
        pass

@bot1.on(events.CallbackQuery(pattern=r'^hm_event_page_(\d+)_(\d+)$'))
async def hmode_event_page_callback(event):
    page = int(event.pattern_match.group(1))
    owner_user_id = int(event.pattern_match.group(2))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    user_doc = await users_catcher_col.find_one({"user_id": owner_user_id}, {"event_filter": 1, "_id": 0}) or {}
    text, buttons = await _hmode_event_screen(owner_user_id, user_doc.get("event_filter"), page=page)
    await event.answer()
    try:
        await event.edit(text, parse_mode='html', buttons=buttons)
    except Exception:
...
@bot1.on(events.CallbackQuery(pattern=r'^hm_event_pick_(\d+)_(\d+)$'))
async def hmode_event_pick_callback(event):
    ...
    if idx < 0 or idx >= len(all_events):
        return await event.answer("❌ ဒီ event ကို မတွေ့တော့ပါ — ပြန်ရွေးပါ။", alert=True)
    chosen_event = all_events[idx]
    await users_catcher_col.update_one({"user_id": owner_user_id}, {"$set": {"event_filter": chosen_event}}, upsert=True)
    await event.answer("…event set")
    await _hmode_finish(event, owner_user_id)

@bot1.on(events.CallbackQuery(pattern=r'^hm_event_skip_(\d+)$'))
async def hmode_event_skip_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    await users_catcher_col.update_one({"user_id": owner_user_id}, {"$set": {"event_filter": None}}, upsert=True)
    await event.answer("…event skipped")
    await _hmode_finish(event, owner_user_id)

@bot1.on(events.CallbackQuery(pattern=r'^hm_back_main_(\d+)$'))
async def hmode_back_main_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    user_doc = await users_catcher_col.find_one({"user_id": owner_user_id}, {"harem_style": 1, "_id": 0}) or {}
    text = _hmode_main_screen(user_doc.get("harem_style"))
    buttons = _hmode_main_buttons(owner_user_id)
    await event.answer()
    try:
        await event.edit(text, parse_mode='html', buttons=buttons)
    except Exception:
...
@bot1.on(events.CallbackQuery(pattern=r'^hm_back_sort_(\d+)$'))
async def hmode_back_sort_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    user_doc = await users_catcher_col.find_one({"user_id": owner_user_id}, {"harem_style": 1, "_id": 0}) or {}
    style = user_doc.get("harem_style") or "default"
    text, buttons = _hmode_sort_screen(style, owner_user_id)
    await event.answer()
    try:
        await event.edit(text, parse_mode='html', buttons=buttons)
    except Exception:
...
@bot1.on(events.CallbackQuery(pattern=r'^hm_back_rarity_(\d+)$'))
async def hmode_back_rarity_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    user_doc = await users_catcher_col.find_one({"user_id": owner_user_id}, {"rarity_filter": 1, "_id": 0}) or {}
...
@bot1.on(events.CallbackQuery(pattern=r'^hm_close_(\d+)$'))
async def hmode_close_callback(event):
    owner_user_id = int(event.pattern_match.group(1))
    if event.sender_id != owner_user_id:
        return await event.answer("⚠️ ဒါက သင့်ရဲ့ setting မဟုတ်ပါ။", alert=True)
    await event.answer()
...
async def send_paginated_harem(client, chat_id, user_id, page=1, edit_msg_id=None, viewer_id=None):
    ...
    # Projected — this function only ever reads harem/rarity_filter/event_filter/harem_style/
    # fav_card off the user doc, no need to pull wallet_balance, cooldowns, msg_history, etc. too.
    user_doc = await users_catcher_col.find_one(
        {"user_id": user_id},
        {"harem": 1, "rarity_filter": 1, "event_filter": 1, "harem_style": 1, "fav_card": 1, "_id": 0}
    ) or {}
    ...
    # 🎪 /hmode's event picker (see _hmode_event_screen) — like rarity_filter, this genuinely
    # RESTRICTS what shows up here, not just a sort priority. Unlike rarity_filter, harem items
    # never stored their own "event" snapshot (see the $push at /collect time), so this can't be
    # applied to raw_harem/filtered_harem the way rarity_filter is above — it's applied further
    # down, against db_chars, once each owned char_id's CURRENT event tag is known.
    event_filter = user_doc.get("event_filter")
    harem_style = user_doc.get("harem_style") or "default"
    ...
    if is_unlimited_vault:
        ...
        if not owned_ids:
            ...
            return
    db_chars = await characters_base_col.find(
        {"char_id": {"$in": owned_ids}},
        {"char_id": 1, "name": 1, "category": 1, "rarity": 1, "event": 1, "currency_value": 1, "_id": 0}
    ).to_list(length=None)
    if event_filter:
        db_chars = [c for c in db_chars if (c.get("event") or "").strip().lower() == event_filter.strip().lower()]
        if not db_chars:
            msg = f"⚔️ <b>…nobody matches that, quietly.</b>\nUse <code>/hmode</code> to change or clear it."
            if edit_msg_id: await client.edit_message(chat_id, edit_msg_id, msg, parse_mode='html')
            else: await client.send_message(chat_id, msg, parse_mode='html')
    ...
    for cat in sorted(by_category.keys(), key=lambda c: c.lower()):
        ...
        for card in cat_cards:
            ...
            # 🎛 /hmode's Default vs Detailed interface toggle: Detailed spells the rarity name
            # out instead of just its emoji, and adds the card's CCT value. The page-chunking
            # loop below computes each line's real length dynamically (utf16_len(line)), so a
            # longer Detailed line is still safely accounted for against PAGE_CHAR_BUDGET —
            # no separate budget tuning needed for this mode.
            if harem_style == "detailed":
                rarity_label = RARITY_DISPLAY_NAME.get(tier, "Unknown")
                value_note = f" · 💰{card.get('currency_value', 0)} CCT"
                card_line = (
                    f"<code>{display_char_id(cid)}</code> | {rarity_emoji} {rarity_label} | "
                    f"{escape_html(display_name)} ({status_str}){value_note}"
                )
            else:
                card_line = (
    ...
    if event_filter:
        output_text += f" <b>🎪Event:</b> {escape_html(event_filter)}\n"
    output_text += "\n"
    for l in page_lines:
        output_text += l + "\n"
    output_text += "\n"
    ...
    if rarity_filter or event_filter:
...
def _inline_media_result(builder, media, cid, title, caption, buttons=None):
    ...
    buttons (optional): reply_markup baked into the result itself — Telegram attaches it to
    the message the moment it's sent, no chosen-inline-result round trip required. Left as
    None for harem/dex (unchanged); the name-search mode below is the only caller that uses
    it (see handle_name_search_inline_query)."""
    if isinstance(media, types.MessageMediaPhoto):
        return builder.photo(file=media, id=cid, text=caption, parse_mode='html', buttons=buttons)
...
async def handle_name_search_inline_query(event, term):
    """Bare/unprefixed inline queries — unified_inline_query_handler's fallback below, so this
    runs for anything that ISN'T harem./dex./collected./a /fuck-style command. A whole-roster
    character browser+search: empty text (a fresh "@bot", or tapped straight from /search's
    switch_inline button) shows a random assortment to browse; non-empty text does a substring
    + case-insensitive NAME search ("mikasa" matches a character actually named "Mikasa
    Ackerman" — no need to type the full name). Either way, up to 50 results, shuffled first
    then sorted rarity high→low (so the visible set is fresh each time, not just "the 50
    rarest" every single time).
    The full card (Category/Name/ID/Rarity/Value/Caught-Globally — all stored fields, no extra
    per-card queries) plus the 🎯 My Catches / 🌍 Global Catchers buttons are baked into the
    result right here and land on the message the instant it's selected — no chosen-inline-
    result/raw-update handling needed. (An earlier version deferred this to a
    events.Raw(UpdateBotInlineSend) handler; that subscribed to EVERY update the bot receives —
    not just inline selections — and was almost certainly why the whole bot, not just this
    feature, got dramatically slower. Removed for that reason.)"""
    builder = event.builder
    term = (term or "").strip()
    all_chars = await get_all_characters_cached()
    if term:
        term_lower = term.lower()
        pool = [c for c in all_chars if term_lower in (c.get("name") or "").lower()]
    else:
        pool = list(all_chars)
    random.shuffle(pool)
    page_chars = pool[:50]
    page_chars.sort(key=lambda x: -_rarity_weight_for_sort(x.get("rarity", "")))
    try:
        media_by_cid = await get_char_display_media_batch(bot1, page_chars)
    except Exception as e:
        print(f"Name-search inline gallery batch fetch error: {e}")
        media_by_cid = {}
    results = []
    for card in page_chars:
        cid = card["char_id"]
        ...
        if not storage_media:
            continue
        try:
            caption = _search_card_base_text(card, cid)
            buttons = [[
                Button.inline("🎯 My Catches", data=f"srch_my_{cid}"),
                Button.inline("🌍 Global Catchers", data=f"srch_glb_{cid}")
            ]]
            results.append(_inline_media_result(builder, storage_media, cid, card['name'], caption, buttons=buttons))
        except Exception as e:
            print(f"Name-search inline gallery error for {cid}: {e}")
            continue
    await event.answer(results, cache_time=0)
...
async def unified_inline_query_handler(event):
    ...
    if query_text.startswith("harem."):
        ...
        # 🔍 Anything else — including an EMPTY query, e.g. straight off /search's
        # switch_inline button — falls through to the whole-roster browse/name-search above.
        await handle_name_search_inline_query(event, query_text)

async def _char_top_catchers_leaderboard(char_id, chat_id=None):
    """Top-10 catchers of char_id, ranked by catch count then earliest catch (tiebreaker) —
    same ranking /check already uses for its own global leaderboard (kept as a separate,
    parallel implementation here so this doesn't touch /check's working code at all).
    chat_id=None means GLOBAL (every catch, anywhere); a real chat_id restricts to catches
    whose harem entry recorded that chat_id — populated by perform_catch on every real
    spawn-catch (gift/trade transfers don't set it, which is correct: nothing was "caught in a
    chat" there). Returns the formatted "➥ name x N" block, or None if nobody qualifies."""
    if chat_id is None:
        match_stage = {"harem.char_id": char_id}
        filter_cond = {"$eq": ["$$item.char_id", char_id]}
    else:
        match_stage = {"harem": {"$elemMatch": {"char_id": char_id, "chat_id": chat_id}}}
        filter_cond = {"$and": [{"$eq": ["$$item.char_id", char_id]}, {"$eq": ["$$item.chat_id", chat_id]}]}
    pipeline = [
        {"$match": match_stage},
        {"$project": {
            "fullname": "$fullname", "user_id": "$user_id",
            "matches": {"$filter": {"input": "$harem", "as": "item", "cond": filter_cond}}
        }},
        {"$project": {
            "fullname": 1, "user_id": 1,
            "count": {"$size": "$matches"}, "first_caught": {"$min": "$matches.caught_date"}
        }},
    ...
    for u in top_hunters:
        ...
        out += f"➥ {mention} x{u['count']}\n"
    return out

def _search_card_base_text(character, char_id):
    rarity_tier = classify_rarity(character.get("rarity", ""))
    rarity_emoji = RARITY_EMOJI.get(rarity_tier, RARITY_DEFAULT_EMOJI)
    rarity_plain = RARITY_DISPLAY_NAME.get(rarity_tier, "Unknown")
    return (
        f"OwO! Check out this character!\n\n"
        f"{escape_html(character.get('category') or 'Unknown Series')}\n"
        f"{display_char_id(char_id)}: {escape_html(character['name'])}\n"
        f"({rarity_emoji} <b>RARITY</b>: {rarity_plain})\n\n"
        f"💰 <b>Value:</b> {character.get('currency_value', 0)} CCT\n\n"
        f"🌎 <b>Caught Globally:</b> {character.get('spawn_count', 0)} times"
    )

@bot1.on(events.CallbackQuery(pattern=r'^srch_glb_(\d+)$'))
async def search_global_catchers_callback(event):
    char_id = int(event.pattern_match.group(1))
    await event.answer()
    character = await characters_base_col.find_one({"char_id": char_id}, {"_id": 0})
    if not character:
        return
    leaderboard = await _char_top_catchers_leaderboard(char_id, chat_id=None)
    section = (
        f"\n\n🎖️ <b>TOP 10 GLOBAL CATCHERS OF THIS CHARACTER</b>\n{leaderboard}"
        if leaderboard else "\n\n🎖️ <i>No global catchers yet.</i>"
    )
    text = _search_card_base_text(character, char_id) + section
    try:
        await event.edit(text, parse_mode='html', buttons=None)
    except Exception as e:
        print(f"search_global_catchers_callback edit error: {e}")

@bot1.on(events.CallbackQuery(pattern=r'^srch_my_(\d+)$'))
async def search_my_catches_callback(event):
    """Replaces the originally-planned per-chat 'GROUP CATCH' leaderboard (impossible for
    inline-sent messages — see inline_search_result_chosen's docstring for why) with a personal
    stat instead, per owner's choice: how many times the TAPPING user themselves has caught
    this exact character, anywhere. Only needs event.sender_id, which — unlike a chat_id — IS
    always reliably present on a CallbackQuery regardless of whether it came from a normal or
    an inline-sent message."""
    char_id = int(event.pattern_match.group(1))
    await event.answer()
    character = await characters_base_col.find_one({"char_id": char_id}, {"_id": 0})
    if not character:
        return
    user_id = event.sender_id
    user_doc = await users_catcher_col.find_one({"user_id": user_id}, {"harem": 1, "_id": 0}) or {}
    my_count = sum(
        1 for item in user_doc.get("harem", [])
        if isinstance(item, dict) and item.get("char_id") == char_id
    )
    section = (
        f"\n\n🎯 <b>You've caught this character {my_count} time{'s' if my_count != 1 else ''}!</b>"
        if my_count > 0 else "\n\n🎯 <i>You haven't caught this character yet!</i>"
    )
    text = _search_card_base_text(character, char_id) + section
    try:
        await event.edit(text, parse_mode='html', buttons=None)
    except Exception as e:
        print(f"search_my_catches_callback edit error: {e}")
...
# two actual replies the owner supplied). Two things are deliberately NOT identical because we
# genuinely don't have the same data catch_bot does:
#   • Value shows our flat currency_value, not a random min~max range — nothing in this bot's
#     catch flow ever rolls a random reward off of it (see perform_catch), so a fake range
#     would just be made-up numbers.
...
async def check_character_id_handler(event):
    ...
    info_text = (
        ...
        f"({rarity_emoji} {RARITY_LABEL_STYLED}: {rarity_plain})"
        f"{event_block}\n\n"
        f"💰 V{catchstyle_caps('alue')}: {character.get('currency_value', 0)} CCT\n\n"
...
# ---- /search : entry point into the whole-roster character NAME search/browse (see
# handle_name_search_inline_query) — no longer does an anime/category lookup; /dex <category>
# already covers that exactly the same way /search used to, so nothing is lost by dropping it
# here. Bare /search just offers a button into the browse-random inline mode; /search <text>
# prefills that text as the search term so tapping the button jumps straight to matches. ----
@bot1.on(events.NewMessage(pattern=own_pattern(r'^[/.]search(?:@\w+)?$', 'bot1')))
async def search_entry_handler(event):
    await event.reply(
        "🔵 <b>To search character click on button below</b>",
        parse_mode='html',
        buttons=[[Button.switch_inline("🔍 Search Characters", query="", same_peer=True)]]
    )

@bot1.on(events.NewMessage(pattern=own_pattern(r'^[/.]search(?:@\w+)?\s+(.+)$', 'bot1')))
async def search_with_text_handler(event):
    term = event.pattern_match.group(1).strip()
    await event.reply(
        f"🔵 <b>Tap below to search characters matching \"{escape_html(term)}\"</b>",
        parse_mode='html',
        buttons=[[Button.switch_inline(f"🔍 Search \"{term}\"", query=term, same_peer=True)]]
    )
...
async def dex_with_category_handler(event):
    ...
    if not matched_cat:
        return await event.reply(
            ...
            f"💡 Use <code>/dex</code> to see all categories, or search by character name below.",
            parse_mode='html',
            buttons=[[Button.switch_inline(f"🔍 Search \"{category_query}\"", query=category_query, same_peer=True)]]
        )
