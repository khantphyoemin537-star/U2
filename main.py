    ...
    await users_catcher_col.update_one(
        {"user_id": user_id},
        {
            "$setOnInsert": {
                ...
                "harem": [],
                "fullname": fullname,
                "ccm_balance": 0,  # 💰 CCM CURRENCY SYSTEM — see /balance, /daily, /setvalue
                "daily_cooldown": 0,
                "hunt_cooldown": 0,
                "last_daily": 0,
                "daily_streak": 0,
...
# Divine (RARITY_GATE_TIERS, the quiz-gated tier — see there) spawns, relative to its own
# default below. RARE and below are never touched by this — only the quiz-gated tier scales.
...
#
# 🩹 CHANGED (owner report — explicit reversal of the previous change): the PREVIOUS version
# here was deliberately "opposite of normal gacha weighting" — DIVINE/MYSTICAL/LEGENDARY were
# the DOMINANT tiers (~97% of organic spawns combined), RARE/UNCOMMON/COMMON pushed down to a
# residual sliver. That's exactly backwards from what "rarity" is supposed to mean, and is what
# the owner was reporting (Mystical/Divine spawning the most). Now a proper curve: each tier is
# roughly HALF as likely as the one below it, so rarity actually predicts frequency again —
# COMMON is genuinely the most common, DIVINE genuinely the rarest of this group. If you want a
# steeper or gentler curve, just adjust the numbers below directly (see the percentage note
# further down for exactly what each number currently works out to).
DEFAULT_RARITY_WEIGHTS = dict(zip(RARITY_TIERS, [0, 0, 0, 1, 2, 4, 8, 16, 32, 0, 0, 0]))
...
# 🩹 CHANGED (2026-09, owner request — "SUPREME/CATAPHRACT keep spawning all the time, make it
# much rarer so the top rarities are actually worth something"): 1 -> 0.2, AND weights are now
# applied PER TIER instead of per character (see pick_character_by_tier). The per-character
# version is the real reason they spawned so often: every SUPREME/CATAPHRACT/CROSSVERSE card in
# the roster carried its own weight of 1, so a tier with hundreds of cards was hundreds of times
# more likely than its "weight 1" suggested.
TOP_TIER_WEIGHT_OFF = 0.2  # SUPREME/CATAPHRACT/CROSSVERSE weight when /topspawn is off (default)
...
# Weights are PER TIER (pick_character_by_tier rolls the tier first, then a random card inside
# it), so these percentages are what a chat actually sees no matter how many cards each tier has.
# At level 1, /topspawn off (default, total weight 63.6): COMMON ≈50.3%, UNCOMMON ≈25.2%,
# RARE ≈12.6%, LEGENDARY ≈6.3%, MYSTICAL ≈3.1%, DIVINE ≈1.6%, and SUPREME/CATAPHRACT/CROSSVERSE
# ≈0.3% EACH (≈0.9% combined, roughly 1 spawn in 106 for the three together). With /topspawn on
# (total weight 363) they jump to ≈27.5% each. Tune the numbers above directly for a different
# curve — /spawnrates (owner) prints the live per-tier percentages for the current roster and
# settings, so a change can be checked without waiting for spawns.
...
def pick_character_by_tier(candidates, tier_weights):
    """Rolls the TIER first (using tier_weights, restricted to tiers that actually have at least
    one candidate), then picks uniformly among that tier's candidates. This is what makes the
    weights above mean what they say: a tier's chance no longer scales with how many cards it
    happens to contain. If every present tier has weight 0 (shouldn't happen), falls back to a
    plain uniform pick rather than raising."""
    by_tier = {}
    for c in candidates:
        tier = c.get("rarity_tier") or classify_rarity(c.get("rarity", ""))
        by_tier.setdefault(tier, []).append(c)
    tiers = list(by_tier.keys())
    weights = [tier_weights.get(t, UNKNOWN_TIER_WEIGHT) for t in tiers]
    if not tiers:
        return None
    if sum(weights) <= 0:
...
def get_spawn_eligible_characters(characters_list):
    """The organic-spawn pool: skips /addspecial CNFT cards (spawnable=False) and anything that
    already hit its CatchLimit. Shared by trigger_dynamic_spawn and /spawnrates so the two can't
...
async def _generate_new_char_id():
    ...
    with no locking between reading that max and using it. userbot_channel_listener fires
    ...
    return f"BOD{counter_doc['value']}"
...
# ==========================================
# 💰 CCM CURRENCY SYSTEM — reward math
# ==========================================
# Brings the currency system back (per owner request), under the name "ccm" (💰). Two owner-
# configurable pricing tables live in bot_settings_col as singleton docs, both edited through
# /setvalue — see that command (and its SUPREME event-picker flow) near perform_catch below:
#
#   • "ccm_tier_values" — Rarity 2 (CATAPHRACT) through 9 (COMMON) each get a flat [min, max]
#     range; a catch of that tier pays a random amount inside it. Keyed by RARITY_NUM_MAP's own
#     "2".."9" numbering so there's never a second name<->number translation to keep in sync.
#   • "ccm_event_values" — Rarity 1 (SUPREME) is priced per EVENT instead of one flat range,
#     since SUPREME characters are event-exclusive drops and different events (e.g. an
#     anniversary drop vs. a routine one) can be worth very different amounts. Stored as a list
#     of {"event","value"} pairs rather than a dict keyed by event name, so an event name
#     containing "." can never be misread as a Mongo dotted field path.
#
# Neither table needs to be fully populated for the bot to keep working — a tier/event with no
# value set yet just pays 0 (compute_ccm_reward's caller hides the "+0 ccm" line entirely in
# that case), same fail-open philosophy perform_catch already uses everywhere else.
DAILY_CCM_REWARD = 100  # 💰 flat /daily payout — no streak scaling, per owner spec

async def _get_ccm_tier_values():
    """{"2": {"min","max"}, ..., "9": {...}} — only tiers the owner has actually /setvalue'd
    are present; a missing key means "not priced yet", not "priced at zero"."""
    doc = await bot_settings_col.find_one({"_id": "ccm_tier_values"})
    return (doc or {}).get("values", {})

async def _get_ccm_event_values():
    """{event_name: value} for SUPREME (Rarity 1) — reshaped from the stored
    {"events": [{"event","value"}, ...]} list into a dict, which is all any caller needs."""
    doc = await bot_settings_col.find_one({"_id": "ccm_event_values"})
    return {e["event"]: e["value"] for e in (doc or {}).get("events", []) if e.get("event")}

async def _set_ccm_event_value(event_name, value):
    """Upsert a single event's ccm value into the ccm_event_values list (see comment above for
    why this is a list of pairs rather than a dict-keyed-by-event-name doc)."""
    doc = await bot_settings_col.find_one({"_id": "ccm_event_values"}) or {"events": []}
    ...
    for e in events_list:
        ...
        events_list.append({"event": event_name, "value": value})
    await bot_settings_col.update_one({"_id": "ccm_event_values"}, {"$set": {"events": events_list}}, upsert=True)

async def _compute_ccm_reward(spawn_data):
    """How much 💰 ccm a just-caught character is worth, right now, per whatever the owner has
    live in /setvalue. Never raises and never blocks a catch — a tier/event that hasn't been
    priced yet simply pays 0."""
    tier = classify_rarity(spawn_data.get("rarity"))
    if tier == "SUPREME":
        event_values = await _get_ccm_event_values()
        return int(event_values.get(spawn_data.get("event") or "General", 0))
    num = RARITY_TIER_TO_NUM.get(tier)  # None for CNFT / OTHER — those aren't priced by this system
    if not num:
        return 0
    rng = (await _get_ccm_tier_values()).get(num)
    if not rng:
        return 0
    ...
    async with spawn_locks[chat_id]:
        ...
        try:
            ...
            for attempt in range(max_attempts):
                ...
                chosen_char = pick_character_by_tier(candidates, RARITY_WEIGHTS)
...
TRIVIA_DIFFICULTY_CONFIG = {
    "EASY":         {"label": "Easy",         "emoji": "🟢", "weight": 12, "ccm_min": 200,  "ccm_max": 400,  "points": 5,  "wrong_penalty": 2,  "ccm_penalty": 40},
    "NORMAL":       {"label": "Normal",       "emoji": "🔵", "weight": 27, "ccm_min": 450,  "ccm_max": 650,  "points": 8,  "wrong_penalty": 4,  "ccm_penalty": 90},
    "HARD":         {"label": "Hard",         "emoji": "🟠", "weight": 28, "ccm_min": 700,  "ccm_max": 900,  "points": 12, "wrong_penalty": 6,  "ccm_penalty": 140},
    "VERY_HARD":    {"label": "Very Hard",    "emoji": "🔴", "weight": 20, "ccm_min": 950,  "ccm_max": 1200, "points": 18, "wrong_penalty": 9,  "ccm_penalty": 190},
    "EXTREME_HARD": {"label": "Extreme Hard", "emoji": "🟣", "weight": 13, "ccm_min": 1250, "ccm_max": 1500, "points": 25, "wrong_penalty": 13, "ccm_penalty": 250},
}
...
async def sync_from_catch_command(event):
    ...
    status_msg = await event.reply(
        ...
        f"Paced at {CATCHBOT_SYNC_DELAY_SECONDS}s/request. Open-ended: it keeps going until "
        f"{CATCHBOT_SYNC_CONSECUTIVE_MISS_STOP} consecutive empty ids in a row — i.e. until it "
        f"actually reaches catch_bot's real last id, whatever that currently is — so this can "
        f"take a while. If the bot restarts, it'll resume from here automatically next time it "
        f"boots, or whenever you run /syncfromcatch again.",
        parse_mode='html'
    )
...
async def sync_from_catch_parallel_command(event):
    ...
    status_msg = await event.reply(
        ...
        f"🚦 Channel Work paused — automatic spawns paused until this finishes.\n"
    ...
    # 🚦 Same real pause /syncfromcatch uses — see CHANNEL_WORK_ENABLED above. Always restored
    # in the finally block below, even if a worker raises or the run is cancelled midway.
    await _set_channel_work(False, reason="syncfromcatch_parallel")
    try:
        ...
        await _set_channel_work(True, reason="syncfromcatch_parallel finished")
...
async def _execute_char_merge(groups, cmd_name):
    """Shared merge executor for /mergenamedupes — same canonical-selection,
    ...
    for char_ids in groups.values():
        ...
        try:
            ...
            total_spawn_count = sum(d.get("spawn_count", 0) for d in docs)
            await characters_base_col.update_one({"char_id": canonical}, {"$set": {"spawn_count": total_spawn_count}})
            for dup_id in duplicates:
                result = await users_catcher_col.update_many(
                    {"harem.char_id": dup_id},
                    {"$set": {"harem.$[elem].char_id": canonical}},
                ...
                if dup_doc and dup_doc.get("storage_msg_id"):
                    try:
                        await bot1.delete_messages(SPECIFIC_CONTROL_GROUP, [dup_doc["storage_msg_id"]])
                    except Exception:
                        pass
...
POWER_RANGER_COLLECTIONS = [
    ...
    # 🥷 SovereignNinja's pool: its /addninja saves {"name": ..., "session": <StringSession>} into
    # db["telegram_bot"]["ninja_col"] — the same database name bot1 uses (see `db = client_mongo[...]`
    # near the top), so this loads the very same accounts as long as bot1's MONGO_URI env var points
    # at the same Mongo cluster as SovereignNinja's.
    "ninja_col",
]
WORKER_POOL_LOAD_CONCURRENCY = 6   # sessions connected at the same time while loading
# The ninja sessions were created with SovereignNinja's API_ID/API_HASH. Telegram normally accepts a
# session under a different api_id too, so by default this just reuses bot1's APP_ID/APP_HASH — but if
# loads fail with an api-related error, set NINJA_API_ID / NINJA_API_HASH env vars to SovereignNinja's.
_WORKER_API_ID = int(os.environ.get("NINJA_API_ID") or APP_ID)
_WORKER_API_HASH = os.environ.get("NINJA_API_HASH") or APP_HASH
worker_pool_clients: List[Tuple[TelegramClient, str]] = []
_worker_pool_load_lock = asyncio.Lock()
# Live/last-load info so commands can say WHY the pool is empty instead of just "empty":
# still loading, no collection had any docs, or every session failed (and with what error).
_worker_pool_status: Dict[str, Any] = {"loading": False, "done": 0, "total": 0, "collections": {}, "failures": []}

async def _worker_pool_collection_names() -> List[str]:
    return list(POWER_RANGER_COLLECTIONS)

def _worker_pool_short_error(e: Exception) -> str:
    if type(e).__name__ == "AuthKeyDuplicatedError":
        return ("AuthKeyDuplicatedError — this session is being used from another IP/process "
                "(stop the ninja host / anything else running it, then the session may need re-generating)")
    return f"{type(e).__name__}: {str(e)[:80]}"

def _worker_pool_report_text(max_fail: int = 6) -> str:
    """HTML summary of the last pool load: docs found per collection + failed sessions."""
    st = _worker_pool_status
    cols = ", ".join(f"{escape_html(str(k))}={v}" for k, v in st["collections"].items()) or "—"
    out = f"🗂️ Collections (docs): <code>{cols}</code>"
    fails = st["failures"]
    if fails:
        out += f"\n⚠️ <b>{len(fails)} session(s) failed to load:</b>"
        for f in fails[:max_fail]:
            out += f"\n  • <code>{escape_html(f)}</code>"
        if len(fails) > max_fail:
            out += f"\n  …+{len(fails) - max_fail}"
    return out

def _worker_pool_empty_text() -> str:
    """Why-is-the-pool-empty message (HTML) used by /nsync, /nsyncwarmup, etc."""
    st = _worker_pool_status
    if st["loading"]:
        return (f"⏳ <b>Worker pool ကို load လုပ်နေဆဲ</b> (<code>{st['done']}/{st['total']}</code>) — "
                f"session ၃၀ ကျော်ဆိုရင် ခဏကြာတယ်၊ ပြီးမှ ပြန်စမ်းပါ။")
    return ("❌ <b>Worker pool empty.</b>\n" + _worker_pool_report_text() +
            "\n\n<i>ninja_col=0 ဆိုရင် bot1 ရဲ့ MONGO_URI env က SovereignNinja သုံးတဲ့ Mongo cluster နဲ့ မတူတာ ဖြစ်နိုင်တယ် "
            "(database နာမည်က နှစ်ခုလုံး telegram_bot)။ Session error ပါရင် အပေါ်က အကြောင်းရင်းကို ကြည့်ပါ။ "
            "ပြင်ပြီးရင် </i><code>/xbotloadworkers</code>")

async def load_worker_pool():
    """Connects every saved session found in POWER_RANGER_COLLECTIONS (incl. SovereignNinja's
    ninja_col) and keeps the ones that log in in `worker_pool_clients`. Safe to call again later
    (e.g. via /xbotloadworkers) — it disconnects and rebuilds the pool each time instead of
    appending duplicates.

    Doc schema (what SovereignNinja's /addninja writes): {"name": str, "session": "<StringSession>"}.
    Sessions are loaded a few at a time (WORKER_POOL_LOAD_CONCURRENCY) with connect() rather than
    start(): start() on a dead session tries to prompt for a phone number on stdin, connect() +
    is_user_authorized() just reports it. Anything that fails is recorded in _worker_pool_status.
    ⚠️ Never run this while ANOTHER process is logged in with the same sessions — Telegram
    kills a session used from two IPs at once (AuthKeyDuplicatedError)."""
    global worker_pool_clients
    async with _worker_pool_load_lock:
        st = _worker_pool_status
        st.update(loading=True, done=0, total=0, collections={}, failures=[])
        try:
            ...
            candidates = []   # (col_name, doc, session_str)
            seen_sessions = set()
            for col_name in await _worker_pool_collection_names():
                count = 0
                try:
                    async for doc in db[col_name].find({}):
                        count += 1
                        session_str = doc.get("session") or doc.get("session_string") or doc.get("string_session")
                        if not session_str or session_str in seen_sessions:
                            continue
                        seen_sessions.add(session_str)
                        candidates.append((col_name, doc, session_str))
                except Exception as e:
                    print(f"⚠️ [worker pool] couldn't read db['{col_name}']: {e}")
                    st["failures"].append(f"{col_name}: cannot read collection ({type(e).__name__})")
                st["collections"][col_name] = count
                print(f"🗂️ [worker pool] db['{col_name}']: {count} doc(s)")
            st["total"] = len(candidates)

            sem = asyncio.Semaphore(WORKER_POOL_LOAD_CONCURRENCY)

            async def _load_one(col_name, doc, session_str):
                label = str(doc.get("name") or doc.get("user_id") or doc.get("_id"))
                client = None
                async with sem:
                    try:
                        client = TelegramClient(StringSession(session_str), _WORKER_API_ID, _WORKER_API_HASH)
                        ...
                        return (client, f"{label} (@{me.username or me.id})")
                    except Exception as e:
                        reason = _worker_pool_short_error(e)
                        print(f"❌ [worker pool] Failed to load {col_name}/{label}: {reason}")
                        st["failures"].append(f"{col_name}/{label}: {reason}")
            ...
            results = await asyncio.gather(*[_load_one(c, d, ss) for c, d, ss in candidates])
            worker_pool_clients = [r for r in results if r]
        finally:
            st["loading"] = False
        print(f"🚀 [worker pool] {len(worker_pool_clients)} worker(s) ready for /xbotresync + /xbotimportall + /nsync.")
...
async def xbot_load_workers_command(event):
    """(Re)loads the worker sessions (powerranger_* + SovereignNinja's ninja_col) into
    worker_pool_clients. Run this once after deploying (or any time you add/remove a session) —
    /xbotresync, /xbotimportall and /nsync pick up whatever's loaded here."""
    if event.sender_id != OWNER_ID:
        return
    if _worker_pool_status["loading"]:
        return await event.reply(_worker_pool_empty_text(), parse_mode='html')
    status = await event.reply("⏳ Loading worker pool (powerranger_* + ninja_col)...")
    await load_worker_pool()
    if worker_pool_clients:
        names = "\n".join(f"  • {escape_html(label)}" for _client, label in worker_pool_clients[:45])
        more = f"\n  …+{len(worker_pool_clients) - 45}" if len(worker_pool_clients) > 45 else ""
        await status.edit(
            f"✅ <b>Loaded {len(worker_pool_clients)} worker(s)</b>\n{_worker_pool_report_text()}\n\n{names}{more}",
            parse_mode='html',
        )
    else:
        await status.edit(_worker_pool_empty_text(), parse_mode='html')
...
# ==========================================
# /setvalue (owner-only) sets Rarity 2-9's [min,max] ccm ranges in one shot, then walks
# straight into the SUPREME (Rarity 1) per-event picker below. /balance and /daily are the
# player-facing side. See _compute_ccm_reward / _get_ccm_tier_values / _get_ccm_event_values
# near CNFT_RARITY_INFO for the actual pricing data these all read and write.
# ==========================================
_SETVALUE_LINE_RE = re.compile(r'(?m)^\s*([2-9])\s+(\d+)\s+(\d+)\s*$')
...
async def _send_ccm_event_picker(event):
    ...
    reply with that event's ccm value (see ccm_event_picker_pick / ccm_event_value_reply)."""
    ...
    if not event_names:
        ...
        return
    configured = await _get_ccm_event_values()
    text = (
        "🪞 <b>SUPREME (Rarity 1) — Event အလိုက် ccm Value</b>\n\n"
        "Event တစ်ခုချင်းစီအတွက် SUPREME character ဖမ်းမိရင် ရမယ့် ccm ကို သတ်မှတ်ပါ။\n"
...
async def setvalue_cmd(event):
    ...
    # moment. Either way, one line per tier: "<Rarity No. 2-9> <Min> <Max>".
    ...
    if not matches:
        current = await _get_ccm_tier_values()
        if not current:
            return await event.reply(
                "📋 <b>Rarity 2–9 ccm value ဘာမှ သတ်မှတ်ရသေးပါ။</b>\n\n"
                "<b>Usage:</b> line တစ်ကြောင်းစီ <code>[Rarity No.] [Min] [Max]</code>, Rarity 2 to 9 "
                "အားလုံးအတွက် — /setvalue ရဲ့ text အနေနဲ့ တန်းရိုက်ထည့်ပါ၊ သို့မဟုတ် table ပါတဲ့ message ကို "
                "Reply ပြန်ပြီး <code>/setvalue</code> ရိုက်ပါ:\n\n"
                "<pre>2 5000 10000\n3 4000 8000\n4 3000 6000\n5 2000 4000\n6 1000 2000\n"
                "7 500 1000\n8 200 500\n9 100 300</pre>",
                parse_mode='html'
            )
        lines = [
            f"<code>{n}</code> {RARITY_NUM_MAP[n]['name']} — <code>{v['min']:,}–{v['max']:,}</code> ccm"
            for n, v in sorted(current.items(), key=lambda kv: int(kv[0]))
        ]
        return await event.reply(
            f"📋 <b>လက်ရှိ Rarity ccm Values:</b>\n<blockquote>{chr(10).join(lines)}</blockquote>\n\n"
            f"ပြောင်းချင်ရင် <code>/setvalue</code> နောက်မှာ table အသစ်ကို ရိုက်ထည့်ပါ (or reply ပို့ပါ)။",
            parse_mode='html'
        )
    ...
    await event.reply(
        f"✅ <b>{len(updates)} Rarity tier(s) ccm Value သတ်မှတ်ပြီးပါပြီ:</b>\n"
...
async def ccm_event_picker_done(event):
    ...
    try:
        await event.edit("✅ <b>SUPREME event ccm values ပြီးပါပြီ။</b>", parse_mode='html', buttons=None)
    except errors.MessageNotModifiedError:
        pass
    await event.answer()
...
async def ccm_event_picker_pick(event):
    ...
    try:
        ...
        current = (await _get_ccm_event_values()).get(event_name)
        hint = f" (လက်ရှိ: <code>{current:,}</code> ccm)" if current is not None else ""
        text = (
            f"🪞 <b>Event:</b> {escape_html(event_name)}{hint}\n\n"
            f"↩️ <b>Reply to this message</b> with the ccm value for this event."
        )
        sent = await event.reply(text, parse_mode='html')
...
async def ccm_event_value_reply(event):
    ...
    raw = (event.text or "").strip()
    if not raw or not raw.lstrip("-").isdigit() or int(raw) < 0:
        pending_ccm_event_prompt_ids[event.reply_to_msg_id] = event_name  # invalid — keep waiting
        await event.reply("❌ ဂဏန်း (positive number) ပဲ ပို့ပါ — ↩️ ဒီ Reply ကိုပဲ ပြန် Reply လုပ်ပါ။", parse_mode='html')
        return
    value = int(raw)
    await _set_ccm_event_value(event_name, value)
    await event.reply(f"✅ <b>{escape_html(event_name)}</b> ➜ <code>{value:,}</code> ccm သတ်မှတ်ပြီးပါပြီ။", parse_mode='html')
...
async def ccm_balance_cmd(event):
    ...
    user_doc = await users_catcher_col.find_one({"user_id": user_id})
    ccm_balance = (user_doc or {}).get("ccm_balance", 0)
    title_text = escape_html(f"{plain_name}'s Wallet")
    text = f"💰 <b>{title_text}</b>\n\n💰 <b>ccm:</b> <code>{ccm_balance:,}</code>"
    await event.reply(text, parse_mode='html')
...
async def ccm_daily_cmd(event):
    ...
    user_doc = await users_catcher_col.find_one({"user_id": user_id})
    now = time.time()
    last_daily = (user_doc or {}).get("last_daily", 0)
    streak = (user_doc or {}).get("daily_streak", 0)
    if now - last_daily < 86400:
        rem_time = int(86400 - (now - last_daily))
        return await event.reply(
            f"⏳ {mention} <b>ယနေ့အတွက် ယူပြီးသားပါ။</b> နောက်တစ်ကြိမ် <code>{str(timedelta(seconds=rem_time))}</code> အတွင်း ပြန်ယူလို့ရပါမယ်။",
            parse_mode='html'
        )
    ...
    await users_catcher_col.update_one(
        {"user_id": user_id},
        {"$inc": {"ccm_balance": DAILY_CCM_REWARD}, "$set": {"last_daily": now, "daily_streak": new_streak, "fullname": plain_name}}
    )
    await event.reply(
        f"🎁 {mention} <b>Daily Bonus ရပါပြီ! <code>+{DAILY_CCM_REWARD:,} 💰 ccm</code></b>\n"
...
# actual transfer goes through try_deduct_ccm (see near _compute_ccm_reward) so a race between
...
async def gift_ccm_handler(event):
    ...
    if not event.is_reply:
        return await event.reply(
            "↩️ <b>ccm ပေးချင်တဲ့ user ရဲ့ message ကို Reply ပြန်ပြီး</b> <code>/giftccm [amount]</code> <b>ရိုက်ပါ။</b>",
            parse_mode='html'
        )
    is_cd, remaining = await is_on_cooldown(sender_id, "giftccm", GIFTCCM_COOLDOWN_SECONDS)
    if is_cd:
        return await event.reply(f"⏳ <b>{remaining:.0f}s ထပ်စောင့်ပါ။</b>", parse_mode='html')

    replied = await event.get_reply_message()
    if not replied or not replied.sender_id:
        return await event.reply("❌ <b>ဒီ user ကို မတွေ့ပါ။</b>", parse_mode='html')
    receiver_id = replied.sender_id
    if receiver_id == sender_id:
        return await event.reply("❌ <b>ကိုယ့်ကိုယ်ကို ccm ပေးလို့ မရပါဘူး။</b>", parse_mode='html')
    try:
        receiver_sender = await replied.get_sender()
        if getattr(receiver_sender, 'bot', False):
            return await event.reply("❌ <b>Bot ကို ccm ပေးလို့ မရပါဘူး။</b>", parse_mode='html')
    except Exception:
    ...
    if amount < GIFTCCM_MIN_AMOUNT:
        return await event.reply(f"❌ <b>အနည်းဆုံး {GIFTCCM_MIN_AMOUNT} ccm ဖြစ်ရပါမယ်။</b>", parse_mode='html')

    sender_doc = await users_catcher_col.find_one({"user_id": sender_id})
    sender_balance = (sender_doc or {}).get("ccm_balance", 0)
    if sender_balance < amount:
        return await event.reply(
            f"❌ <b>ccm မလုံလောက်ပါ။</b> လက်ရှိလက်ကျန်: <code>{sender_balance:,}</code> ccm",
            parse_mode='html'
        )

    sender_mention = await get_html_mention(event, sender_id)
    receiver_mention = await get_html_mention(event, receiver_id)
    confirm_text = (
        f"💰 {sender_mention} ➜ {receiver_mention}\n\n"
        f"<b>{amount:,} ccm</b> လှူဒါန်းမှာ သေချာပါသလား?"
    )
    buttons = [[
        Button.inline("✅ Confirm", data=f"giftccm_confirm_{sender_id}_{receiver_id}_{amount}"),
        Button.inline("…not yet", data=f"giftccm_cancel_{sender_id}_{receiver_id}_{amount}")
    ]]
...
async def gift_ccm_callback_handler(event):
    ...
    if event.sender_id != sender_id:
        return await event.answer("❌ ဒါက သင့်အတွက် မဟုတ်ပါ။", alert=True)
    if not claim_single_tap(event):
        return await event.answer("⏳ Processing...", alert=False)

    if action == "cancel":
        try:
            await event.edit("…changed your mind. ccm stays with you.", buttons=None)
    ...
    try:
        if not await try_deduct_ccm(sender_id, amount):
            await event.edit("❌ <b>ccm မလုံလောက်တော့ပါ။</b>", parse_mode='html', buttons=None)
        ...
        receiver_mention = await get_html_mention(event, receiver_id)
        await event.edit(
            f"✅ {sender_mention} ➜ {receiver_mention}\n💰 <b>{amount:,} ccm</b> လွှဲပြောင်းပြီးပါပြီ!",
            parse_mode='html',
            buttons=None
        )
        await event.answer(f"Sent {amount:,} ccm!", alert=True)
    except Exception as e:
...
async def perform_catch(chat_id, user_id, spawn_data, event, reply_to_msg=None, is_callback=False, temp_msg_id=None):
    ...
    await ensure_user_registered(user_id, plain_name)
    try:
        # 💰 CCM CURRENCY SYSTEM — see _compute_ccm_reward near CNFT_RARITY_INFO. Computed
        # BEFORE the update below so it can ride along in the same atomic $inc rather than a
        # separate DB round trip; 0 (tier/event not priced yet by /setvalue) just skips the
        # "ccm_balance" key entirely below instead of writing a no-op $inc of 0.
        ccm_reward = await _compute_ccm_reward(spawn_data)
        inc_fields = {
            "total_caught": 1,
...
async def handle_harem_inline_query(event, query_text, force_real_harem=False):
    ...
    db_chars = sorted(db_chars, key=lambda x: (
        ...
        -latest_caught.get(x.get("char_id"), 0) if (filter_tier and classify_rarity(x.get("rarity", "")) == filter_tier) else 0,
...
async def check_character_id_handler(event):
    ...
    # 💰 ccm value line (per owner request) — the configured RANGE/flat amount from /setvalue,
    # not a random roll like _compute_ccm_reward returns on an actual catch (a "/check" display
    # showing a different number every time it was viewed would be confusing, not informative).
    # Omitted entirely if the owner hasn't priced that tier/event yet — same "hide, don't show
    # a fake 0" rule the catch success message and /triviatop already follow.
    ccm_value_line = ""
    if rarity_tier == "SUPREME":
        event_val = (await _get_ccm_event_values()).get(character.get("event") or "General")
        if event_val is not None:
            ccm_value_line = f"💰 {catchstyle_caps('worth')}: <code>{event_val:,}</code> ccm\n\n"
    else:
        num = RARITY_TIER_TO_NUM.get(rarity_tier)
        rng = (await _get_ccm_tier_values()).get(num) if num else None
        if rng and int(rng.get("max", 0)) > 0:
            ccm_value_line = f"💰 {catchstyle_caps('worth')}: <code>{int(rng['min']):,}–{int(rng['max']):,}</code> ccm\n\n"
...
async def market_menu_from_catch_callback(event):
    """The 🛒 Buy & Sell button under a fresh catch — same claim-check shape as catchprofile_*."""
    owner_uid = int(event.pattern_match.group(1))
    if event.sender_id != owner_uid:
        return await event.answer("❌ This isn't your catch to act on.", alert=True)
    if not await _market_require_group_member(event):
        return
    await event.answer()
    await _send_market_menu(event, event.sender_id)
...
async def _market_offer_listing_confirm(send, seller_id, char_id, price):
    ...
    char_item = next((x for x in seller_harem if isinstance(x, dict) and x.get("char_id") == char_id and x.get("status") == "vault"), None)
    if not char_item:
...
async def _render_market_sellpick_page(user_id, page):
    ...
    counts = {}
    for item in raw_harem:
        if isinstance(item, dict) and item.get("char_id") and item.get("status") == "vault":
            counts[item["char_id"]] = counts.get(item["char_id"], 0) + 1
    if not counts:
        return ("🛒 <b>Nothing to sell</b> — every copy you own is already listed, or your harem's empty.",
...
async def market_sell_confirm_callback(event):
    ...
    try:
        ...
        if not char_item:
            ...
            await event.answer("Card not found.", alert=True)
            return
        flipped = await set_one_harem_copy_status(seller_id, char_id, "vault", "market")
        if not flipped:
            await event.edit("❌ <b>…she's already gone.</b> Nothing was listed.", parse_mode='html', buttons=None)
            await event.answer("Card not found.", alert=True)
            return
...
async def toggle_top_tier_spawn(event):
    ...
    if new_state:
        text = (
            ...
            f"expect them a LOT more often (<code>/spawnrates</code> shows the live percentages).\n"
...
async def show_spawn_rates(event):
    ...
    await event.reply(
        ...
        f"<i>/topspawn {status} · /spawnweight level {_cached_spawnweight_level} · {len(eligible)} eligible cards</i>\n\n"
...
#   • OVERWRITES every ID in the range by default (same as /syncfromcatch does);
#     `/nsync 1 7215 resume` instead skips ids already refreshed during the last run
#   • A worker whose warm-up fails hands its ids to the healthy workers (rescue pass)
# ==========================================================
...
# 🩹 FIX: `logger` was never defined anywhere in this file (only logging.basicConfig at ERROR
# level exists), so every logger.info/warning call below raised NameError. This gives nsync its
# own INFO-level logger so its progress shows in the console even though the root logger is ERROR.
logger = logging.getLogger("nsync")
logger.setLevel(logging.INFO)
logger.propagate = False
if not logger.handlers:
    _nsync_handler = logging.StreamHandler()
    _nsync_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
    logger.addHandler(_nsync_handler)

# catch_bot's @username — optional override. A freshly loaded userbot session can't resolve a
# bare numeric id until it has "met" the bot, so warm-up resolves it by @username first.
# If this env var is empty (default) the username is AUTO-DETECTED from monitor_userbot, which
# already talks to catch_bot (see _nsync_detect_target_username); if that isn't available either,
# SovereignNinja's own default (NSYNC_DEFAULT_TARGET_USERNAME below) is used.
SYNC_TARGET_BOT_USERNAME = os.environ.get("SYNC_TARGET_BOT_USERNAME", "")
NSYNC_DEFAULT_TARGET_USERNAME = "Character_Catcher_Bot"   # SovereignNinja's own default — last-resort fallback
_nsync_target_username: Optional[str] = None
_nsync_last_error: Dict[str, str] = {}   # worker label -> why its last warm-up failed
...
_nsync_state = {
    ...
    "started_at": None,
    "task": None,
    # 🩹 FIX: defaulted to 0, which made /nsync refuse to run ("Control group not set") until
    # /nsyncsetgroup was used. bot1 reads stored media back out of SPECIFIC_CONTROL_GROUP, so
    # that's the only sensible default (a saved /nsyncsetgroup value still overrides it).
    "control_group_id": SPECIFIC_CONTROL_GROUP,
    "lock": asyncio.Lock(),
    "orphans": [],               # ids whose worker failed warm-up/crashed — re-dealt to healthy workers
    "healthy": [],               # (client, label) of workers that passed warm-up this run
    "warmup_failed": {},         # label -> reason
    "rescued": 0,
    "skipped": 0,
    "mode": "overwrite",         # "overwrite" | "resume"
}
...
async def _nsync_detect_target_username() -> Optional[str]:
    """catch_bot's @username: env override first, else asked from monitor_userbot (which already
    has a working chat with catch_bot, so it can resolve CATCH_BOT_ID). Cached after first hit."""
    global _nsync_target_username
    if _nsync_target_username:
        return _nsync_target_username
    env = (SYNC_TARGET_BOT_USERNAME or "").strip().lstrip("@")
    if env:
        _nsync_target_username = env
        return env
    try:
        if monitor_userbot is not None and monitor_userbot.is_connected():
            ent = await asyncio.wait_for(monitor_userbot.get_entity(CATCH_BOT_ID), timeout=20)
            uname = getattr(ent, "username", None)
            if uname:
                _nsync_target_username = uname
                logger.info(f"🎯 [nsync] catch_bot username auto-detected: @{uname}")
                return uname
    except Exception as e:
        logger.warning(f"[nsync] username auto-detect failed: {type(e).__name__}: {str(e)[:80]}")
    return NSYNC_DEFAULT_TARGET_USERNAME   # not cached, so a later successful auto-detect still wins

async def _nsync_ensure_entities(client, label: str) -> Optional[str]:
    """Makes sure THIS session can address catch_bot (and CATCHBOT_SYNC_CHAT_ID if it's a
    different chat). Telethon StringSessions don't persist their entity cache, so after every
    restart a bare numeric id is unknown until it's resolved once. Order: already cached ->
    resolve @username -> scan this account's dialogs. Returns None on success, else a short reason."""
    needed = [CATCH_BOT_ID]
    if CATCHBOT_SYNC_CHAT_ID != CATCH_BOT_ID:
        needed.append(CATCHBOT_SYNC_CHAT_ID)

    async def all_known() -> bool:
        for cid in needed:
            try:
    ...
    uname = await _nsync_detect_target_username()
    if uname:
        for _ in range(2):
            try:
                await asyncio.wait_for(client.get_input_entity(uname), timeout=20)
                break
            except FloodWaitError as e:
                errs.append(f"FloodWait {e.seconds}s resolving @{uname}")
                if e.seconds > 30:
                    break
                await asyncio.sleep(e.seconds + 1)
            except Exception as e:
                errs.append(f"@{uname}: {type(e).__name__}: {str(e)[:50]}")
                break
        if await all_known():
            return None
    else:
        errs.append("catch_bot @username unknown")
    ...
    try:
        async def _scan():
            ...
            async for d in client.iter_dialogs(limit=None):
                ...
                if not remaining:
                    return
        await asyncio.wait_for(_scan(), timeout=60)
    except FloodWaitError as e:
        errs.append(f"FloodWait {e.seconds}s reading dialogs")
    except Exception as e:
        errs.append(f"dialogs: {type(e).__name__}: {str(e)[:50]}")
    if await all_known():
        return None
    return "; ".join(errs) or "catch_bot not found in this account's chats"

async def _nsync_warmup_one(client, label: str) -> bool:
    """Resolve catch_bot for this session, then send /start with strict timeouts. True on success.
    On failure the reason is stored in _nsync_last_error[label] (shown by /nsyncdebug, /nsyncwarmup
    and the final NSync report)."""
    err = await _nsync_ensure_entities(client, label)
    if err:
        _nsync_last_error[label] = err
        logger.error(f"❌ [nsync/{label}] cannot address catch_bot: {err}")
        return False
    last = "unknown"
    for _ in range(3):
        try:
            await asyncio.wait_for(client.send_message(CATCH_BOT_ID, "/start"), timeout=NSYNC_WARMUP_TIMEOUT)
            await asyncio.sleep(NSYNC_WARMUP_DELAY)
            _nsync_last_error.pop(label, None)
            logger.info(f"✅ [nsync/{label}] /start OK")
            return True
        except asyncio.TimeoutError:
            last = "/start timed out"
        except FloodWaitError as e:
            ...
            await asyncio.sleep(1)
    _nsync_last_error[label] = last
    logger.error(f"❌ [nsync/{label}] warm-up failed: {last}")
    return False

async def _nsync_load_pool_then_warmup():
    """🩹 FIX: start_system() used to fire load_worker_pool() and _nsync_boot_warmup() as two
    independent tasks, with the warm-up only sleeping 5s. Loading ~31 sessions takes far longer
    than that, so the warm-up saw an EMPTY pool and silently did nothing. Now it's one task:
    load the pool, THEN warm it up."""
    await load_worker_pool()
    if NSYNC_AUTO_WARMUP_ON_BOOT:
        await _nsync_boot_warmup()

async def _nsync_boot_warmup():
    """Warms up every worker in worker_pool_clients (call only after the pool is loaded)."""
    if not worker_pool_clients:
...
async def _nsync_store(client, label: str, info: dict, reply_msg) -> str:
    ...
    Mirrors store_character_from_check (same fields, same cache upkeep) but forwards the media
    with THIS worker's own session — it's the only one holding a valid file_reference for it."""
    char_id = f"BOD{info['id']}"
    ctrl = _nsync_state["control_group_id"]
    try:
        existing = await characters_base_col.find_one({"char_id": char_id})
        r_info = _resolve_check_rarity(info["rarity_raw"], existing)
        if not r_info:
            r_info = RARITY_NUM_MAP[str(len(_NON_CNFT_TIERS))]  # lowest base tier (Common), same fallback as /syncfromcatch

        fwd = await asyncio.wait_for(
            client.send_message(ctrl, "", file=reply_msg.media),
            timeout=60,
        )
        storage_id = fwd.id
        try:
            prewarm_media_identity_cache(fwd, char_id)
        except Exception:
            pass
        ...
        if existing:
            ...
            if old_sid and old_sid != storage_id:
                try:
                    # bot1 (admin of the storage group) deletes — a worker userbot usually
                    # can't delete messages someone else posted.
                    await bot1.delete_messages(ctrl, [old_sid])
                except Exception:
                    pass
...
async def _nsync_process_id(client, label: str, num: int) -> str:
    """One .check -> store cycle for a single id; updates the shared stats. Returns the raw
    outcome ('cancelled' means the caller must stop)."""
    ...
    if outcome == "ok" and reply is not None:
        ...
        if info:
            ...
            key = result if result in ("imported", "updated") else "errors"
            logger.info(f"[nsync/{label}] ✓ id={num} → {result}")
    elif outcome == "miss":
        key = "misses"
    async with _nsync_state["lock"]:
        _nsync_state["stats"]["checked"] += 1
        _nsync_state["stats"][key] += 1
    return outcome

async def _nsync_run_ids(client, label: str, ids: List[int], worker_index: Optional[int]):
    """Runs `ids` one-at-a-time on one warmed-up worker. Anything not reached because of an
    unexpected crash goes back into _nsync_state['orphans'] so another worker can pick it up."""
    pos = 0
    try:
        for pos, num in enumerate(ids):
            if _nsync_state["cancel"]:
                logger.info(f"🛑 [nsync/{label}] cancelled at {pos}/{len(ids)}")
                return
            if await _nsync_process_id(client, label, num) == "cancelled":
                return
            if worker_index is not None:
                _nsync_state["progress"][worker_index] = pos + 1
            await asyncio.sleep(NSYNC_PACE_PER_CHECK)
    except Exception as e:
        logger.exception(f"❌ [nsync/{label}] worker CRASHED at idx {pos}: {e}")
        _nsync_state["orphans"].extend(ids[pos:])
...
async def _nsync_worker(client, label: str, my_ids: List[int], worker_index: int):
    ...
    try:
        if not await _nsync_warmup_one(client, label):
            _nsync_state["warmup_failed"][label] = _nsync_last_error.get(label, "unknown")
            _nsync_state["orphans"].extend(my_ids)   # 🚑 hand the whole range to the healthy workers
            logger.warning(f"⚠️ [nsync/{label}] warm-up failed — {len(my_ids)} ids handed to the rescue pool")
            return
        _nsync_state["healthy"].append((client, label))
        await _nsync_run_ids(client, label, my_ids, worker_index)
        logger.info(f"✅ [nsync/{label}] worker DONE")
    except Exception as e:
        logger.exception(f"❌ [nsync/{label}] worker CRASHED: {e}")
        done = _nsync_state["progress"].get(worker_index, 0)
        _nsync_state["orphans"].extend(my_ids[done:])
...
async def _nsync_run(start_id: int, end_id: int, resume: bool = False):
    """Main run loop — static range split across all worker_pool_clients.
    Default = OVERWRITE every id in the range. resume=True skips ids already refreshed since
    the start of the previous (interrupted) run."""
    if not worker_pool_clients:
        await bot1.send_message(OWNER_ID, _worker_pool_empty_text(), parse_mode='html')
    ...
    if resume:
        try:
            doc = await bot_settings_col.find_one({"_id": "ninja_sync_settings"})
            run_ts = doc.get("last_run_started") if doc else None
        except Exception as e:
            logger.warning(f"[nsync] resume marker lookup failed: {e}")
        if run_ts:
            try:
                cursor = characters_base_col.find(
                    {"synced_via_check": True, "char_id": {"$regex": r"^BOD\d+$"},
                     "last_synced_at": {"$gte": run_ts}},
                    {"char_id": 1},
                )
            ...
            logger.info("[nsync] resume requested but no previous run recorded — doing a full overwrite")
    if not (resume and run_ts):
        # Fresh overwrite run: remember when it began so a later `resume` knows what's already done.
        run_ts = time.time()
        try:
            await bot_settings_col.update_one(
                {"_id": "ninja_sync_settings"},
                {"$set": {"last_run_started": run_ts, "last_run_range": [start_id, end_id]}},
                upsert=True,
            )
        except Exception as e:
            logger.warning(f"[nsync] could not save run marker: {e}")
    _nsync_state["mode"] = "resume" if skip else "overwrite"
    _nsync_state["skipped"] = len(skip)

    all_ids = [i for i in range(start_id, end_id + 1) if i not in skip]
    total = len(all_ids)
    if total == 0:
        try:
            await bot1.send_message(OWNER_ID, "✅ <b>NSync:</b> Nothing left to do — every id in this range was already refreshed in the last run.", parse_mode='html')
        except Exception:
            pass
        _nsync_state["running"] = False
        return
    ...
    _nsync_state["orphans"] = []
    _nsync_state["healthy"] = []
    _nsync_state["warmup_failed"] = {}
    _nsync_state["rescued"] = 0

    logger.info("=" * 60)
    logger.info(f"🚀 [nsync] _RUN START ({_nsync_state['mode']})")
    ...
    try:
        ...
        # 🚑 Rescue pass: ids belonging to workers that failed warm-up (or crashed) are re-dealt
        # round-robin across the workers that ARE healthy. Up to 3 rounds.
        for rnd in range(1, 4):
            if _nsync_state["cancel"] or not _nsync_state["orphans"] or not _nsync_state["healthy"]:
                break
            healthy = list(_nsync_state["healthy"])
            ids = sorted(set(_nsync_state["orphans"]))
            _nsync_state["orphans"] = []
            _nsync_state["rescued"] += len(ids)
            logger.info(f"🚑 [nsync] rescue round {rnd}: {len(ids)} ids over {len(healthy)} healthy workers")
            buckets = _partition_round_robin(ids, len(healthy))
            rescue_tasks = [
                asyncio.create_task(_nsync_run_ids(c, l, b, None))
                for (c, l), b in zip(healthy, buckets) if b
            ]
            await asyncio.gather(*rescue_tasks, return_exceptions=True)
    except Exception as e:
        ...
        failed = dict(_nsync_state["warmup_failed"])
        left = len(_nsync_state["orphans"])
        if _nsync_state["cancel"]:
            header = "🛑 <b>Cancelled</b>"
        elif failed and not _nsync_state["healthy"]:
            header = "❌ <b>Failed — no worker could reach catch_bot</b>"
        else:
            header = "🏁 <b>Finished</b>"
        extra = ""
        if _nsync_state["rescued"]:
            extra += f"🚑 Rescued: <code>{_nsync_state['rescued']}</code>\n"
        if failed:
            extra += f"\n❌ <b>Warm-up failed on {len(failed)}/{num_workers} worker(s):</b>\n"
            for lbl, why in list(failed.items())[:5]:
                extra += f"  • <code>{escape_html(lbl)}</code> — {escape_html(why)}\n"
            if len(failed) > 5:
                extra += f"  …+{len(failed) - 5}\n"
        if left:
            extra += (f"\n⏳ <b>{left} id(s) were NOT processed.</b> Fix the cause, then run "
                      f"<code>/nsync {start_id} {end_id} resume</code> to finish only what's left.\n")
        try:
            await bot1.send_message(
                OWNER_ID,
                f"{header} <b>NSync</b> ({_nsync_state['mode']})\n\n"
...
# ---------- Commands ----------
@bot1.on(events.NewMessage(pattern=own_pattern(r'^[/.]nsync(?:@\w+)?(?:\s+(\d+))?(?:\s+(\d+))?(?:\s+(resume))?$', 'bot1')))
async def nsync_cmd(event):
    ...
    resume = bool(event.pattern_match.group(3))
    ...
    _nsync_state["task"] = asyncio.create_task(_nsync_run(start_id, end_id, resume))

    await event.reply(
        f"🔄 <b>NSync Started</b> — {'♻️ RESUME (already-refreshed ids skipped)' if resume else '✍️ OVERWRITE all ids'}\n"
...
async def nsync_status(event):
    ...
    elapsed = int(time.time() - (_nsync_state["started_at"] or time.time()))
    wl = []
    failed = _nsync_state["warmup_failed"]
    for i in range(min(10, len(_nsync_state["ranges"]))):
        d = _nsync_state["progress"].get(i, 0)
        t = len(_nsync_state["ranges"][i])
        pct = (d / t * 100) if t else 0
        bf = int(pct / 10)
        lbl = worker_pool_clients[i][1] if i < len(worker_pool_clients) else ""
        wl.append(f"  [{i+1}] {'█'*bf}{'░'*(10-bf)} <code>{d}/{t}</code>{' ❌' if lbl in failed else ''}")
    extra = ""
    if failed:
        extra += f"\n❌ warm-up failed: <code>{len(failed)}</code> worker(s) — ids go to the rescue pool"
    if _nsync_state["orphans"]:
        extra += f"\n🚑 waiting for rescue: <code>{len(_nsync_state['orphans'])}</code>"
    await event.reply(
        f"🔄 <b>NSync: RUNNING</b> ({_nsync_state['mode']})\n"
...
async def nsync_debug(event):
    ...
    for i, (c, label) in enumerate(worker_pool_clients[:15], 1):
        try:
            ...
            err = _nsync_last_error.get(label)
            lines.append(f"  {i}. <code>{escape_html(label)}</code> — conn={conn}" + (f" ⚠️ {escape_html(err)}" if err else ""))
        except Exception as e:
            lines.append(f"  {i}. <code>{escape_html(label)}</code> — <code>{type(e).__name__}</code>")
    ...
    await event.reply(
        ...
        f"🎯 target chat: <code>{CATCHBOT_SYNC_CHAT_ID}</code> (@{escape_html(uname) if uname else 'username unknown'})\n"
        f"{'⏳ pool: LOADING ' + str(_worker_pool_status['done']) + '/' + str(_worker_pool_status['total']) if _worker_pool_status['loading'] else '👷 pool: loaded'}\n"
        f"{_worker_pool_report_text()}\n\n"
...
async def _nsync_warmup_bg(status_msg):
    ...
    for i, (c, label) in enumerate(pairs, 1):
        ...
        if success:
            ...
            fail += 1
            lines.append(f"  {i}. ❌ <code>{escape_html(label)}</code> — {escape_html(_nsync_last_error.get(label, '?'))}")
...
async def start_system():
    ...
    asyncio.create_task(_nsync_load_settings())  # 🥷 NSync — restore saved control-group id (non-blocking; catches its own errors)
    ...
    asyncio.create_task(_nsync_load_pool_then_warmup())  # 🥷 = load_worker_pool() followed by the nsync boot warm-up
    global BOT1_READY
