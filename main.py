...
async def build_harem_groups(harem, rarity_filter=None):
    ...
    for cd in char_docs:
        by_series[cd.get("series") or "Unknown"].append(cd)

    groups = []
    for series in sorted(by_series.keys(), key=lambda s: s.lower()):
        chars = by_series[series]

        def sort_key(cd):
            rarity = classify_rarity(cd.get("rarity", "Lower"))
            priority = 0 if (rarity_filter and rarity == rarity_filter) else 1
            return (priority, RARITY_ORDER.get(rarity, 6), (cd.get("name") or "").lower())
        chars.sort(key=sort_key)

        total_in_series = await characters_base_col.count_documents({"series": series})
        lines = []
        for cd in chars:
            rarity = classify_rarity(cd.get("rarity", "Lower"))
            qty = counts.get(cd["char_id"], 0)
            qty_tag = f" (x{qty})" if qty > 1 else ""
            lines.append(
                f"{RARITY_EMOJI.get(rarity, '•')} <code>{escape_html(str(cd['char_id']))}</code> "
                f"{escape_html(cd.get('name', 'Unknown'))}{qty_tag}"
            )
        groups.append({"series": series, "owned": len(chars), "total": total_in_series, "lines": lines})
    return groups
...
async def send_harem_overview(event, user_id, page=1, edit_msg_id=None):
    ...
    groups = await build_harem_groups(harem, rarity_filter)
    pages = paginate_harem_groups(groups)
    ...
    total_cards = len(harem)
    unique_cards = len({c.get("char_id") for c in harem if c.get("char_id")})
    ...
    buttons.append([Button.switch_inline("🔍 Open Vault Inline", query=f"harem.{user_id}", same_peer=False)])
    buttons.append([
        Button.inline(f"👀 Browse Cards ({total_cards})", data=f"hgal_1_{user_id}"),
        Button.inline("🎯 Priority", data=f"goto_hmode_{user_id}")
    ])
...
async def callback_handler(event):
    ...
    if data.startswith("hpage_"):
        ...
        await event.answer()
        msg = await event.get_message()
        await send_harem_overview(event, owner_id, page=page, edit_msg_id=msg.id)
        return
    if data.startswith("hback_"):
        owner_id = int(data.split("_")[1])
        await event.answer()
        msg = await event.get_message()
        await send_harem_overview(event, owner_id, page=1, edit_msg_id=msg.id)
        return
