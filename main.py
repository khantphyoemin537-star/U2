...
FORCE_SUB_GATED_COMMANDS = {
    ...
    "hmode",                   # rarity filter
    "gift",
    "Ifuck", "morgan", "catch", # kept from the original set
}
...
SPAM_CATCH_MUTE_SECONDS = 480  # 8 minutes — how long /Ifuck (and /who, /w, /waifu) stay
...
async def spam_detection_and_mute(event):
    ...
    # user_id alone, so spamming in Group A now blocks /Ifuck and /who in EVERY group, not
    ...
    if user_id in user_mute_until and now < user_mute_until[user_id]:
        if _extract_command_word(event.text) in ['/Ifuck', '/obtain', '/who', '/w', '/waifu']:
            try:
                await event.delete()
            except:
                pass
        ...
        try:
            ...
            await event.respond(
                bq(f"<b>Notice:</b> {mention}, that's enough. "
                   f"You've been blocked from /Ifuck and /who <b>in every group</b> for {mute_minutes} minutes. "
...
async def _identify_media_and_reply(event, media_msg):
    ...
    if best_match:
        ...
        try:
            ...
            fuck_command = f"/Ifuck {best_match['name']}"
            ...
            try:
                buttons = types.ReplyInlineMarkup(rows=[
                    types.KeyboardButtonRow(buttons=[
                        types.KeyboardButtonCopy(text="🤍 /Ifuck", copy_text=fuck_command),
    ...
    # removed — /catch and /Ifuck media is identical now, no need to guess which other bot a
    # spawn "belongs" to)
    ...
    if not event.is_private and chat_id in active_group_spawns:
        ...
        if event.is_reply and event.reply_to_msg_id == spawn_data["spawn_msg_id"]:
            ...
            try:
                ...
                reveal_lines += ["", f"<code>/Ifuck {escape_html(char_name)}</code>", "", "…say it, and I'm yours."]
                reveal_text = "\n".join(reveal_lines)
                try:
...
# from — instead of /Ifuck [name]. Nothing about bot1's own reveal flow changes.
#
...
async def xbot_resync_channel_command(event):
    ...
    if target_chat_id:
        ...
        chat_ids_to_scan = list(monitored_chat_ids)
        status = await event.reply(f"⏳ Resetting all {len(chat_ids_to_scan)} channels... (/xbotresynccancel to stop)")
...
CATCH_USAGE_TEXT = (
    "📌 <b>Usage:</b> <code>/Ifuck [Character Name]</code>\n"
...
# ⚡ PERFORMANCE: send_paginated_harem needs the vault owner's display name for its mention
# line on EVERY render — the first /harem AND every subsequent Next/Previous tap. A fresh
# `client.get_entity(user_id)` call for that on every single one of those is a live Telegram
# API round trip sitting in the critical path of the single most frequently re-rendered view
# in the bot, and is a real contributor to /harem (and hmode-filtered /harem, which renders
# through this exact same function) feeling slow — with or without a rarity filter active,
# every page turn paid this cost. A short TTL is enough: the name is shown to the same person
# across a handful of taps a few seconds apart, not something that needs to be live every time.
HAREM_NAME_CACHE_TTL = 1800  # 30 minutes
_HAREM_NAME_CACHE = {}  # user_id -> (fullname, cached_at)

async def _get_harem_display_name(client, user_id):
    cached = _HAREM_NAME_CACHE.get(user_id)
    if cached and (time.time() - cached[1]) < HAREM_NAME_CACHE_TTL:
        return cached[0]
    try:
    ...
    _HAREM_NAME_CACHE[user_id] = (fullname, time.time())
    return fullname
...
async def send_paginated_harem(client, chat_id, user_id, page=1, edit_msg_id=None, viewer_id=None):
    ...
    if _cached_pages_entry and now_ts < _cached_pages_entry[2]:
        ...
        # 🩹 FIX (per owner report — vault felt slow/tedious to browse): this was 500, leaving
        # roughly half of the 1024-char budget on the table for every page, for every user,
        # every time — which means noticeably MORE pages (more Next taps, each paying the
        # network cost of a page turn) than the caption limit actually requires. The real,
        # worst-case overhead OUTSIDE this budget (the "outside PAGE_CHAR_BUDGET" text added
        # further down) is: the mention line — a Telegram display name can run long, so budget
        # generously for it — plus the optional filter line, the optional "premium owner"/
        # filter-active footer lines, and a couple of blank spacer lines. That tops out well
        # under 300 characters even for a long name, so 700 leaves a comfortable ~300-char
        # safety margin under the real 1024 limit while roughly halving how many pages a large
        # vault (or a heavily hmode-filtered one) needs.
        PAGE_CHAR_BUDGET = 700
    ...
    for gi, (cat, owned_in_cat, total_in_cat, chunk) in enumerate(page_groups):
        ...
        prev_cat = cat
    # ⚡ PERFORMANCE: see _get_harem_display_name above — this used to be a fresh
    # client.get_entity(user_id) call right here, paid on every single render (initial
    # /harem AND every Next/Previous tap), regardless of whether a rarity filter was active.
    fullname = await _get_harem_display_name(client, user_id)
...
# ---- /Ifuck <name> : lets the /who reveal button's switch_inline query actually send a
# message. Accepts legacy "/obtain ", "/collect ", "/catch " and "/fuck " prefixes too, so any
# switch_inline buttons already sent to chats before the /obtain → /fuck → /Ifuck renames still
# work — but always emits the new "/Ifuck <name>" text going forward. ----
async def handle_collect_inline_query(event, query_text):
    for legacy_prefix in ("/Ifuck ", "/fuck ", "/obtain ", "/collect ", "/catch "):
...
async def unified_inline_query_handler(event):
    ...
    elif query_text.startswith(("/Ifuck ", "/fuck ", "/obtain ", "/collect ", "/catch ")):
...
async def unified_callback_handler(event):
    ...
    if action_type == "harem":
        ...
        # 🩹 FIX: this callback never called event.answer(), unlike every other branch in this
        # handler — so the tapped Next/Previous button just sat there showing Telegram's
        # loading spinner until the client gave up on it, instead of clearing the instant the
        # new page was ready. Made every single page turn feel stuck/slow on top of whatever
        # the render itself cost.
        await event.answer()
    # ==========================================
