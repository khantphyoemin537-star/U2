...
async def sync_groups_cmd(event):
    """Owner-only: reconciles active_groups (groups_col — what /send reads) against every
    group Telegram says this bot is actually in right now. Nothing in this file writes to
    groups_col except the ChatAction handler (welcome_goodbye) going forward, so this is the
    one-time catch-up for any chat the bot was already in before that existed, or any drift
    since (e.g. Telegram not delivering a kick/leave update while the bot was offline)."""
    if event.sender_id != OWNER_ID: return
    status = await event.reply("🔄 <b>Syncing active_groups against real Telegram dialogs...</b>", parse_mode='html')
    seen_ids = set()
    added, updated = 0, 0
    async for dialog in bot1.iter_dialogs():
        if not dialog.is_group:
            continue
        seen_ids.add(dialog.id)
        result = await groups_col.update_one(
            {"chat_id": dialog.id},
            {"$set": {"chat_id": dialog.id, "title": dialog.title or "Unknown"}},
            upsert=True
        )
        if result.upserted_id is not None:
            ...
            updated += 1
    # Anything left in groups_col that isn't a live dialog anymore means the bot isn't in that
    # chat (kicked while offline, chat deleted, etc.) — drop it so /send stops retrying it.
    removed = 0
    if seen_ids:
        stale = await groups_col.count_documents({"chat_id": {"$nin": list(seen_ids)}})
        if stale:
            await groups_col.delete_many({"chat_id": {"$nin": list(seen_ids)}})
            removed = stale
    await status.edit(
        f"✅ <b>Group sync complete.</b>\n"
        f"➕ New: <code>{added}</code>\n"
        f"🔄 Updated: <code>{updated}</code>\n"
        f"🗑️ Removed (bot no longer there): <code>{removed}</code>\n"
        f"📊 Total tracked now: <code>{len(seen_ids)}</code>",
        parse_mode='html'
    )
...
def _ccm_event_picker_buttons(event_names, configured):
    ...
    for name in event_names:
        ...
        row.append(Button.inline(f"{mark}{name}", data=f"ccmsetevent_{name}"))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
...
@bot1.on(events.CallbackQuery(pattern=r'^ccmsetevent_(?!done$)(.+)$'))
async def ccm_event_picker_pick(event):
    if event.sender_id != OWNER_ID: return await event.answer()
    event_name = event.pattern_match.group(1)
...
async def send_paginated_harem(client, chat_id, user_id, page=1, edit_msg_id=None, viewer_id=None, fav_idx=0):
    ...
    # Projected — this function only ever reads harem/rarity_filter/fav_card(s) off the user
    # doc, no need to pull wallet_balance, cooldowns, msg_history, etc. too.
    user_doc = await users_catcher_col.find_one({"user_id": user_id}, {"harem": 1, "rarity_filter": 1, "fav_card": 1, "fav_cards": 1, "_id": 0}) or {}
    ...
    # 💠 CNFT promo footer (per owner request): shown under EVERY /harem page — own vault,
    # someone else's, unlimited owner view, any rarity filter — pointing at the "cnft."
    # inline gallery (handle_cnft_inline_query) via the button added below. Plain caption
    # text, not a hardcoded username, so it keeps working no matter which bot account this
    # runs under.
    output_text += (
        f"\n\n✦ 𝐂𝐍𝐅𝐓 𝐂𝐚𝐫𝐝𝐬 𝐚𝐫𝐞 𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞 𝐛𝐞𝐥𝐨𝐰.\n"
        f"ᯓ➤ 𝐉𝐮𝐬𝐭 𝐭𝐚𝐩 𝐭𝐡𝐞 「𝐂𝐍𝐅𝐓」 𝐛𝐮𝐭𝐭𝐨𝐧 𝐭𝐨 𝐯𝐢𝐞𝐰 𝐭𝐡𝐞𝐦."
    )
    buttons = []
    ...
    buttons.append([Button.switch_inline("「CNFT」", query="cnft.", same_peer=True)])
    if not buttons:
        buttons = None
    fav_media = None
