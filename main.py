...
async def _identify_media_and_reply(event, media_msg, known_source_bot=None):
    ...
    try:
        ...
        try:
            # 🩹 Same technique as the cross-bot fallback's button below: Telethon's automatic
            # buttons= classifier doesn't recognize KeyboardButtonCopy as inline-only (see
            # LonamiWebs/Telethon#4588), so build the ReplyInlineMarkup explicitly.
            # BOTH commands are offered — catch_bot and this bot track the same underlying
            # character pool / global catch count (see .check's "caught globally" line), so
            # whichever bot the person actually wants to catch it on, the right copyable
            # command is one tap away. KeyboardButtonCopy only fills the compose box — it
            # doesn't send anything by itself, so tapping "/catch" here is harmless; it only
            # does something once the person actually sends it in a chat catch_bot is in.
            buttons = types.ReplyInlineMarkup(rows=[
                types.KeyboardButtonRow(buttons=[
                    types.KeyboardButtonCopy(text="🤍 /fuck (here)", copy_text=fuck_command),
                    types.KeyboardButtonCopy(text="🎯 /catch (catch_bot)", copy_text=catch_command),
                ])
            ])
